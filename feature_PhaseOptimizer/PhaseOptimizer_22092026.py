...
Usage:
    python PhaseOptimizer_22092026.py <FACILITYID> [options]
    ...
    --min-move-gain float  ย้าย 1 ตัวต้องลด RMS ต่อเฟสได้ ≥ ค่านี้ (kW) [default 0.10]
...
# ต่ำกว่านี้ไม่สั่งเพิ่มเฟสเลย แม้สายจะยาว — ผู้ใช้ไฟน้อยเกินกว่าจะคุ้มค่าก่อสร้าง
DESIGN_MIN_METERS_ABS  = 3
...
# วัดจาก 100 หม้อแปลง: 0.02 → 675 ตัว, 0.10 → 566 ตัว (สมดุลเท่าเดิม, เฟสใหม่ยังมีโหลด),
# 0.15-0.25 ลดงานได้อีกแต่เริ่มมีสายเฟสใหม่ที่ไม่มีมิเตอร์ใช้งาน
MIN_MOVE_GAIN_KW = 0.10
...
# เป้า %Unbalance ภายในตอนกระจายเฟส = สัดส่วนของเกณฑ์ (0.9 x 20% = 18%)
# ต่ำกว่านี้ได้สมดุลดีขึ้นแต่ต้องย้ายมิเตอร์มากขึ้นชัดเจน (0.7 = 14% ย้ายมากกว่า ~50%)
_DESIGN_TARGET_FRAC = 0.9

# เกณฑ์ของ "แต่ละกิ่ง" ตอนกระจายเฟส (% เบี่ยงเบนจากเป้าของกองนั้น)
# แยกจากเกณฑ์ที่หม้อแปลง เพราะกิ่งเล็กโหลดเป็นก้อน ยิ่งตั้งต่ำยิ่งต้องย้ายมิเตอร์มาก
_POOL_DEV_TARGET_PCT = 18.0

# เฟสที่เพิ่งเดินเพิ่มในกิ่งหนึ่ง ต้องรับโหลดอย่างน้อยเท่านี้ของโหลดในกิ่งนั้น
# (กันกรณีสร้างสายเฟสใหม่แล้วไม่มีมิเตอร์ย้ายขึ้นไปใช้เลย)
_NEW_PHASE_MIN_SHARE = 0.20
...
def build_non_full_phase_comps(
    ...
    for nodes in nx.connected_components(g):
        ...
        comps.append(DesignComp(
            ...
            violates=(len(meter_fis) >= DESIGN_MIN_METERS_ABS and
                      (length >= min_len_m or len(meter_fis) >= min_meters)),
        ))
...
def simulate_phase_addition(
    ...
    def _run_comp(comp_nodes, comp_edges, label):
        ...
        # เฟสที่ "เพิ่งเดินเพิ่ม" ในกลุ่มนี้ (เดิมไม่มี) ต้องได้โหลดขั้นต่ำ
        # ไม่งั้นบางกิ่งจะถูกสร้างสายเฟสใหม่ทิ้งไว้โดยไม่มีมิเตอร์ย้ายขึ้นไปใช้เลย
        old_mask = 0
        for _fi_e, _eu, _ev, _epd in comp_edges:
            old_mask |= _epd
        added_pds = [pd for pd in tx_pds if not (old_mask & pd)]
        share_eff = dict(share)
        for pd in added_pds:
            share_eff[pd] = max(share_eff.get(pd, 0.0), _NEW_PHASE_MIN_SHARE)
        _s = sum(share_eff.values()) or 1.0
        share_eff = {pd: v / _s for pd, v in share_eff.items()}
        ...
        target_pct = max(1.0, max_imbalance_pct * _DESIGN_TARGET_FRAC)
        ...
        def _balance_pool(fis: List[int], glob_mode: bool = False) -> None:
            ...
            if tot <= 0:
                return
            even = 1.0 / len(tx_pds)
            tgt = {p: tot * ((1.0 - _POOL_SHARE_W) * even + _POOL_SHARE_W * share_eff[p])
                   for p in tx_pds}
            ...
            for f in sorted(fis, key=lambda x: -kw_of[x]):
                if _dev_pct(acc, tgt, tot) < _POOL_DEV_TARGET_PCT:
                    break                   # กองนี้เข้าเกณฑ์แล้ว (ไม่ดูภาพรวม ณ จุดนี้)
        ...
        # ── เกลี่ย "ทุกกิ่ง" ที่เพิ่งเพิ่มเฟส (กองใหญ่ก่อน) ────────────────────
        # ทุกกิ่งที่ลงทุนเดินสายเพิ่มเฟส ต้องมีการย้ายมิเตอร์ให้ใช้เฟสใหม่จริง
        # จึงไม่หยุดกลางคันเพราะภาพรวมหม้อแปลงผ่านไปแล้ว
...
class LVOptimizer:
    ...
    def __init__(
        ...
        self.design_pa_rejected: bool = False    # Step 3c เสนอแล้วแต่ไม่ผ่านเกณฑ์
    ...
    def run(self) -> None:
        ...
        if self.design_violations:
            ...
            elif self.phase_add_options:
                _bad = self.phase_add_options[0]
                _why = ("เฟสหม้อแปลงจะเกินพิกัด "
                        f"({_bad.result.max_phase_loading_pct:.0f}%)"
                        if _bad.result.converged and not _overload_ok(
                            self.baseline, _bad.result, self.max_phase_loading_pct)
                        else (f"แรงดันจะต่ำกว่าเกณฑ์ ({_bad.result.min_v:.0f}V)"
                              if _bad.result.converged and _bad.result.min_v < self.min_voltage_v
                              else (f"%Unbalance จะแย่ลง {-_bad.delta_imbalance:.0f}%"
                                    if _bad.result.converged else "จำลองไม่ converge")))
                print(f"  ไม่รับข้อเสนอ: {_why} — ข้ามการเพิ่มเฟส")
                self.design_pa_rejected = True
        ...
        if design_pending and self.design_pa_rejected:
            design_pending = []      # ลองใน Step 3c แล้วไม่ผ่านเกณฑ์ — ไม่ต้องจำลองซ้ำ
