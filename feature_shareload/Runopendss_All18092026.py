...
# ผลตรวจสอบคุณภาพข้อมูล GIS ของ convert_json_to_dss_ordered() ครั้งล่าสุด — ไม่ใช่ exception,
# แค่บันทึกไว้ให้ผู้เรียก (batch script/LVOptimizer) ไปรายงานต่อว่าไฟล์ JSON ไหนมีข้อมูลผิดปกติ
# บ้าง แม้จะ clamp/แก้ไขให้จำลองไฟฟ้าได้ถูกต้องแล้วก็ตาม (ต้นตอควรถูกแก้ที่ฐานข้อมูล GIS เอง)
LAST_GIS_ISSUES: List[Dict[str, Any]] = []
...
def build_bfs_order(num_nodes: int, edges: List[Tuple[int, int]], source: int):
    ...
    return new_bus, parent, level, tree_edge_set, parent_edge
...
# ─────────────────────────────────────────────────────────────────────────────
# กฎคัดมิเตอร์ / สายบริการ / จับคู่สายบริการ↔มิเตอร์ — ใช้ร่วมกันระหว่าง
# convert_json_to_dss_ordered() และ TransferOptimizer.NetworkGraph
# (สองโมเดลต้องเห็นมิเตอร์ต่อที่จุดเดียวกัน ไม่งั้น optimizer เลือกเฟสจากสายที่มิเตอร์ไม่ได้ต่ออยู่จริง)
# ─────────────────────────────────────────────────────────────────────────────
...
def select_load_point_indices(features: List[Dict[str, Any]]) -> List[int]:
    ...
    for i, f in enumerate(features):
        ...
        if not peano and not peameter:        # เอาเฉพาะ meter จริง
            continue
        out.append(i)
    return out


def select_service_line_indices(features: List[Dict[str, Any]]) -> List[int]:
    """index ของสายบริการ: polyline SUBTYPECODE=5"""
    out: List[int] = []
    for i, f in enumerate(features):
        if not has_paths(f):
            continue
...
def match_service_lines_to_meters(
    ...
    """
    จับคู่สายบริการกับมิเตอร์ → [(index สายบริการ, index มิเตอร์, index โหนดฝั่งสาย LV), ...]
    ปลายสายบริการที่ห่างมิเตอร์ ≤ svc_match คือฝั่งมิเตอร์ ปลายอีกด้านต่อกับโหนดสาย LV ที่ใกล้ที่สุด
    (ถ้าทั้งสองปลายห่างเท่ากัน ถือปลายแรกเป็นฝั่งมิเตอร์; ไม่มีปลายไหนใกล้มิเตอร์ → ไม่ใช้สายเส้นนั้น)
    """
    if not meter_pts or not node_pts:
        return []
    m_arr = np.array(meter_pts, dtype=float).reshape(-1, 2)
    n_arr = np.array(node_pts, dtype=float).reshape(-1, 2)
    ...
    for j, f in enumerate(service_lines):
        ep = endpoints_from_paths(f)
        if not ep:
            continue
        p1, p2 = ep
        m1, d1 = _nearest(m_arr, p1)
        m2, d2 = _nearest(m_arr, p2)
        if d1 <= d2 and d1 <= svc_match:
            meter_idx, net_pt = m1, p2
        elif d2 < d1 and d2 <= svc_match:
            meter_idx, net_pt = m2, p1
        else:
            continue
        out.append((j, meter_idx, _nearest(n_arr, net_pt)[0]))
    return out
...
def convert_json_to_dss_ordered(
    ...
    global LAST_GIS_ISSUES
    LAST_GIS_ISSUES = []

    def _log_gis_issue(kind: str, tag: str, detail: str):
        LAST_GIS_ISSUES.append({"type": kind, "tag": tag, "detail": detail})
    ...
    # Backbone LC (line) — เฉพาะ SUBTYPECODE=1 (สายจ่ายไฟ LV)
    # SUBTYPECODE=2 = สายไฟสาธารณะ (ตัวนำเดี่ยว 1-25AW ไม่มีนิวทรัล) ห้ามนับเป็นสายจ่ายไฟ:
    #   - สายไฟสาธารณะเฟสเดียวที่ออกจากหม้อแปลงจะกลายเป็นช่วงแรกของโครงข่าย → ตัดเฟสอื่นทิ้งทั้งหมด
    #   - หม้อแปลง SUBTYPECODE=1 อ่านเฟส LV จากสายที่ออกจากหม้อแปลง → ไปเจอสายไฟสาธารณะแล้วจำลองผิดเฟส
    # เงื่อนไขต้องเหมือน TransferOptimizer.NetworkGraph._subtype_ok ทุกประการ (โครงข่ายสองฝั่งต้องตรงกัน)
    def _lc_subtype_ok(f: Dict[str, Any]) -> bool:
    ...
    lc_lines = [f for f in features
                if has_paths(f) and "LC" in str(get_attr(f, "TAG", "")).upper() and _lc_subtype_ok(f)]
    if not lc_lines:
        raise RuntimeError("ไม่พบสาย backbone: TAG มี 'LC'")

    # Loads (SUBTYPE=1 point) - exclude transformer point  (กฎร่วมกับ NetworkGraph)
    load_points = [features[i] for i in select_load_point_indices(features)]

    if not load_points:
        raise RuntimeError("ไม่พบ load meters: SUBTYPECODE=1 (point) หลังจากกรอง XF")

    # Service (SUBTYPE=5 line)
    service_lines = [features[i] for i in select_service_line_indices(features)]
    ...
    new_bus, parent, level, tree_edge_set, parent_edge = build_bfs_order(len(reps), edges, source_node)
    ...
    # หม้อแปลงบางตัวเป็น 1/2 เฟส (open-phase bank ต่อคร่อมเฟส-นิวทรัลจริง) ไม่ใช่ 3 เฟสเต็ม
    # เช็คจาก PHASEDESIGNATION ของหม้อแปลงเอง (7=ABC คือ 3 เฟสเต็ม, อย่างอื่นคือ 1/2 เฟส)
    tx_phase_nodes = phase_nodes_from_designation(get_attr(xfmr, "PHASEDESIGNATION", 7))
    tx_is_full3ph = len(tx_phase_nodes) >= 3

    # SUBTYPECODE ของหม้อแปลงเอง: 1 = หม้อแปลง 1 เฟสตัวเดียว (core เดียว, rated เต็มตาม RATEKVA;
    # PHASEDESIGNATION ที่มี 2 บิตหมายถึงเฟส MV 2 เส้นที่คร่อมทางปฐมภูมิเท่านั้น ไม่ใช่ 2 วงจรทุติยภูมิ),
    # 3 = หม้อแปลง 3 เฟส. ค่าอื่น (เช่น bank 2 คอร์อิสระ) ให้ fallback ไปโมเดลแยกก้อนตาม PHASE{A,B,C}_KVA
    try:
        tx_subtypecode = int(get_attr(xfmr, "SUBTYPECODE", -999) or -999)
    except Exception:
        tx_subtypecode = -999
    tx_is_single_unit = (tx_subtypecode == 1) and not tx_is_full3ph

    _phase_kva_field = {1: "PHASEA_KVA", 2: "PHASEB_KVA", 3: "PHASEC_KVA"}
    tx_unit_kva: Dict[int, float] = {}
    for _node in tx_phase_nodes:
        _v = get_attr(xfmr, _phase_kva_field[_node], 0.0)
        try:
            _v = float(_v or 0.0)
        except Exception:
            _v = 0.0
        tx_unit_kva[_node] = _v
    if not tx_is_full3ph and sum(tx_unit_kva.values()) <= 0.0:
        # ไม่มีข้อมูล kVA ต่อเฟส -> แบ่ง rate_kva เท่า ๆ กันตามจำนวนเฟสที่ใช้งานจริง
        _share = rate_kva / max(len(tx_phase_nodes), 1)
        tx_unit_kva = {n: _share for n in tx_phase_nodes}
    ...
    # --- SUBTYPECODE=1: ดูสาย LV (LC) ที่ออกจากหม้อแปลงจริง (feeder head) เพื่อรู้ว่า
    # หม้อแปลงจ่ายเป็นระบบไหน แทนที่จะเดาจาก PHASEDESIGNATION ของหม้อแปลงเอง หรือนับจำนวนมิเตอร์ต่อเฟส
    # สายออก 1 เฟส -> ระบบ 1 เฟส 2 สาย (node เดียว)
    # สายออก 2 เฟส -> ระบบ 1 เฟส 3 สาย (ต้องจำลองทั้ง 2 node พร้อมกัน แบบ bank แยกต่อเฟส)
    if tx_is_single_unit:
        def _valid_pd(v):
            try:
                iv = int(v)
            except Exception:
                return None
            return iv if iv in (1, 2, 3, 4, 5, 6, 7) else None

        _lv_feeder_nodes: set = set()
        _lv_feeder_has_data = False
        for _ei, _ in incident:
            _pd_val = _valid_pd(get_attr(lc_records[_ei][0], "PHASEDESIGNATION", None))
            if _pd_val is not None:
                _lv_feeder_has_data = True
                _lv_feeder_nodes.update(phase_nodes_from_designation(_pd_val))

        tx_lv_single_node = None
        if _lv_feeder_has_data and len(_lv_feeder_nodes) >= 2:
            # สาย LV ออก >=2 เฟส (1 เฟส 3 สาย) -> ห้ามยุบเหลือ node เดียว ให้ไปโมเดลแบบ bank แยกต่อเฟส
            tx_is_single_unit = False
            tx_phase_nodes = sorted(_lv_feeder_nodes)
            tx_unit_kva = {
                n: (tx_unit_kva.get(n) or (rate_kva / len(tx_phase_nodes)))
                for n in tx_phase_nodes
            }
        elif _lv_feeder_has_data and len(_lv_feeder_nodes) == 1:
            tx_lv_single_node = next(iter(_lv_feeder_nodes))
    ...
    # ── Phase clamping (แก้ PHASEDESIGNATION ผิดที่ทำให้เฟส "เพิ่ม" เมื่อไกลจากหม้อแปลง) ──
    # ในทางกายภาพจำนวนเฟสจะลดได้อย่างเดียวเมื่อไกลจากแหล่งจ่าย (ไม่มีจุดเพิ่มเฟสกลางสาย)
    # พบ 110/11,538 หม้อแปลง (0.95%) ที่ segment ปลายสายถูกแท็กเฟสมากกว่า segment ต้นสาย
    # (เช่น สายต้นทางเป็นเฟสเดียว แต่สายปลายทางถูกแท็กเป็น 3 เฟส) — ถ้าปล่อยไว้ OpenDSS จะแก้
    # สมการเฟสที่ "โผล่มาเอง" นี้เป็นเฟสที่ไม่มีเส้นทางไฟฟ้าจริงป้อน ทำให้แรงดันจำลองพังเหลือ
    # ไม่กี่โวลต์ทั้งที่หน้างานไม่มีปัญหา — clamp เฟสของแต่ละ segment ให้เป็นสับเซตของเฟส segment
    # ต้นทาง (parent) เสมอ ไล่จากหม้อแปลงออกไปตามลำดับ BFS
    # ใช้ parent_edge[] จาก build_bfs_order ตรง ๆ (ไม่ผ่าน dict lookup ด้วย (u,v) คู่โหนด) —
    # เส้นซ้ำ/หลายเส้นระหว่างโหนดคู่เดียวกัน (ข้อมูล GIS จริงมี) ทำให้ dict แบบ (u,v)->ei ชี้ผิด
    # เส้นได้ ถ้าเผลอชี้ไปเส้นที่ไม่ใช่ tree edge จริง node นั้นจะไม่ถูก clamp เลย (fallback ไป
    # 3 เฟสเต็มเงียบ ๆ) แล้วบั๊กเดิมจะย้อนกลับมา
    node_eff_phase: Dict[int, set] = {source_node: set(tx_phase_nodes)}
    for _n in sorted((n for n in range(len(reps)) if level[n] >= 0), key=lambda n: level[n]):
        if _n == source_node:
            continue
        _ei = parent_edge[_n] if _n < len(parent_edge) else -1
        if _ei == -1:
            continue
        _seg_f = lc_records[_ei][0]
        _raw = set(phase_nodes_from_designation(get_attr(_seg_f, "PHASEDESIGNATION", 7)))
        _parent_set = node_eff_phase.get(parent[_n], set(tx_phase_nodes))
        if not _raw.issubset(_parent_set):
            _log_gis_issue(
                "phase_increase",
                str(get_attr(_seg_f, "TAG", "") or f"LC_{_ei+1}"),
                f"segment นี้แท็กเฟส {sorted(_raw)} แต่สายต้นทาง (ใกล้หม้อแปลงกว่า) มีแค่เฟส "
                f"{sorted(_parent_set)} — เฟสจะลดได้อย่างเดียวเมื่อไกลจากหม้อแปลง ใช้ "
                f"{sorted(_raw & _parent_set) or sorted(_parent_set)} แทน",
            )
        node_eff_phase[_n] = _raw & _parent_set or _parent_set
    ...
    for ei, (f, u, v) in enumerate(lc_records):
        ...
        _raw_phs = set(phase_nodes_from_designation(pd))
        # clamp กับเฟสจริงของ "ทั้งสองปลาย" เสมอ (ไม่ใช่แค่ tree edge) — self-loop/loop edge
        # ที่เหลือ raw phase ไว้เฉย ๆ จะทำให้ OpenDSS เห็น node ของเฟสที่ไม่มีทางไฟจริงป้อน
        # กลายเป็น sub-node ลอย (ไม่มีกระแสอ้างอิง) แล้วแก้สมการได้ค่าแรงดันเพี้ยนแบบสุ่ม
        _eff = _raw_phs & node_eff_phase.get(u, set(tx_phase_nodes)) & node_eff_phase.get(v, set(tx_phase_nodes))
        # tree edge ที่ไม่ตรงถูก log ไปแล้วตอนคำนวณ node_eff_phase ด้านบน — ที่เหลือ log เฉพาะ
        # loop/self-loop edge (ei ไม่อยู่ใน tree_edge_set) ที่ยังเจอเฟสไม่ตรงกัน กันซ้ำ
        if ei not in tree_edge_set and _eff != _raw_phs:
            _log_gis_issue(
                "phase_inconsistent_loop_edge", str(get_attr(f, "TAG", "") or f"LC_{ei+1}"),
                f"segment นี้ (loop/self-loop) แท็กเฟส {sorted(_raw_phs)} แต่ปลายทาง "
                f"มีเฟสจริงแค่ {sorted(_eff) if _eff else '(ไม่มีเฟสร่วมกันเลย)'}",
            )
        phs = sorted(_eff) if _eff else phase_nodes_from_designation(pd)

        # SHAPE.LEN คำนวณจาก geometry เส้นเองโดย GIS จึงถูกเสมอ — MEASURELENGTH เป็น field
        # แยกที่บันทึกตอนสร้าง/แก้ไขเส้น พบว่า 182/11,538 หม้อแปลง (1.6%) มีค่านี้เพี้ยน/ค้าง
        # จากการแก้ไขที่ไม่ recalculate (ต่างจาก SHAPE.LEN ได้ถึง 3-1,000,000+ เท่า) ทำให้ความยาว
        # สายผิดมหาศาลและแรงดันตกจำลองผิดเพี้ยนไปด้วย — ใช้ SHAPE.LEN เป็นหลัก
        _shape_len = get_attr(f, "SHAPE.LEN", None)
        _measure_len = get_attr(f, "MEASURELENGTH", None)
        if (isinstance(_shape_len, (int, float)) and _shape_len > 2.0
                and isinstance(_measure_len, (int, float)) and _measure_len > 0):
            _ratio = _measure_len / _shape_len
            if _ratio > 3 or _ratio < 0.33:
                _log_gis_issue(
                    "measurelength_mismatch", str(get_attr(f, "TAG", "") or f"LC_{ei+1}"),
                    f"MEASURELENGTH={_measure_len:.1f} แต่ SHAPE.LEN={_shape_len:.1f}m "
                    f"(ต่างกัน {_ratio:.1f} เท่า) — ใช้ SHAPE.LEN แล้ว แต่ควรแก้ MEASURELENGTH ที่ต้นทาง",
                )
        length_m = _shape_len if isinstance(_shape_len, (int, float)) else _measure_len
        length_km = float(length_m or 0.0) / 1000.0

        ctype = get_attr(f, "CONDUCTORTYPE", "")
        csize = get_attr(f, "CONDUCTORSIZE", 25)
        if str(ctype).strip().upper() != "AW" or int(float(csize or 0)) not in AW_IMP:
            _log_gis_issue(
                "conductor_not_in_table", str(get_attr(f, "TAG", "") or f"LC_{ei+1}"),
                f"CONDUCTORTYPE={ctype!r} CONDUCTORSIZE={csize!r} ไม่อยู่ในตาราง impedance "
                f"(รองรับเฉพาะ AW 50/95mm²) — ใช้ค่าประมาณ fallback",
            )
    ...
    svc_cmds = []
    used_svc = set()
    # จับคู่ด้วยกฎร่วม (match_service_lines_to_meters) — NetworkGraph ใช้กฎเดียวกัน
    for j0, meter_idx, net_node in match_service_lines_to_meters(meter_pts, service_lines, reps, svc_match):
        j = j0 + 1
        f = service_lines[j0]
        ...
        # เฟสที่มิเตอร์แจ้ง (PHASE/PHASEDESIGNATION ของตัวมิเตอร์เอง) อาจไม่ตรงกับเฟสจริงที่มี
        # อยู่ ณ จุดต่อบนสาย backbone (หลัง clamp แล้ว) — ถ้าไม่ตรง ให้ยึดเฟสจริงของ backbone
        # เป็นหลัก (มิเตอร์ต้องต่อกับเฟสที่มีไฟจริงเท่านั้น) แก้ทั้ง service line และ Load ให้ตรงกัน
        _avail = node_eff_phase.get(net_node)
        if _avail and pnode not in _avail:
            _log_gis_issue(
                "meter_phase_mismatch", m["load_name"],
                f"มิเตอร์แจ้งเฟส {pnode} แต่จุดต่อบนสาย backbone มีเฟสจริงแค่ {sorted(_avail)} "
                f"— ใช้เฟส {min(_avail)} แทน (ควรแก้ PHASE/PHASEDESIGNATION ของมิเตอร์นี้ที่ GIS)",
            )
            pnode = min(_avail)
            m["pnode"] = pnode
        ...
        _svc_shape_len = get_attr(f, "SHAPE.LEN", None)
        _svc_measure_len = get_attr(f, "MEASURELENGTH", None)  # ดู comment เหตุผลที่บรรทัด ~598
        if (isinstance(_svc_shape_len, (int, float)) and _svc_shape_len > 2.0
                and isinstance(_svc_measure_len, (int, float)) and _svc_measure_len > 0):
            _svc_ratio = _svc_measure_len / _svc_shape_len
            if _svc_ratio > 3 or _svc_ratio < 0.33:
                _log_gis_issue(
                    "measurelength_mismatch", tag,
                    f"MEASURELENGTH={_svc_measure_len:.1f} แต่ SHAPE.LEN={_svc_shape_len:.1f}m "
                    f"(ต่างกัน {_svc_ratio:.1f} เท่า) — ใช้ SHAPE.LEN แล้ว แต่ควรแก้ MEASURELENGTH ที่ต้นทาง",
                )
        length_m = _svc_shape_len if isinstance(_svc_shape_len, (int, float)) else _svc_measure_len
        length_km = float(length_m or 0.0) / 1000.0
    ...
    # Bus coordinates: ให้ odss.Bus.X()/Y() คืนค่าที่ถูกต้องสำหรับทุก backbone bus
    # ต้องรันหลัง Solve จึงจะมี bus list ครบ (source bus เป็น special bus ไม่ต้อง set)
    busxy_cmds = []
    for internal_nid, busname in new_bus.items():
        if busname.lower() in ("source", "sourcebus"):
            continue
        if internal_nid < len(reps):
            x, y = reps[internal_nid]
            busxy_cmds.append(f"SetBusXY Bus={busname} x={x:.3f} y={y:.3f}")

    # Transformer DSS block: 3-phase เต็ม -> 1 ก้อนตามเดิม
    # SUBTYPECODE=1 (1 เฟสตัวเดียว) -> 1 ก้อน rated เต็ม RATEKVA ต่อที่ node ที่มีโหลดจริง
    # อื่น ๆ (bank หลายคอร์) -> แยกเป็นหม้อแปลง 1 เฟสต่อเฟสตาม PHASE{A,B,C}_KVA
    if tx_is_full3ph:
        tx_dss_lines = [
    ...
    elif tx_is_single_unit:
        # เฟสจริงของ node เดียวนี้: ใช้เฟสของสาย LV ที่ออกจากหม้อแปลงเป็นหลัก (ground truth)
        # ถ้าไม่มีข้อมูล PHASEDESIGNATION บนสาย LV เลย ค่อย fallback ไปนับจำนวนมิเตอร์ต่อเฟส
        if tx_lv_single_node is not None:
            tx_single_node = tx_lv_single_node
        else:
            _node_count: Dict[int, int] = {}
            for _m in meter_meta:
                _node_count[_m["pnode"]] = _node_count.get(_m["pnode"], 0) + 1
            tx_single_node = max(_node_count, key=_node_count.get) if _node_count else (tx_phase_nodes[0] if tx_phase_nodes else 1)

        hv_kv_ln = hv_kv / math.sqrt(3.0)
        lv_kv_ln = basekv_lv / math.sqrt(3.0)
        tx_dss_lines = [
            f"New Transformer.{tx_name} Phases=1 Windings=2 Xhl=2.72 conns=(wye, wye) %noloadloss={pct_noloadloss:.6f}",
            f"~ wdg=1 bus=SourceBus.{tx_single_node}.0 conn=wye kv={hv_kv_ln:.6f} kva={rate_kva:.2f} %r={pct_r_each:.6f}",
            f"~ wdg=2 bus=source.{tx_single_node}.0 conn=wye kv={lv_kv_ln:.6f} kva={rate_kva:.2f} %r={pct_r_each:.6f}",
        ]
    else:
        hv_kv_ln = hv_kv / math.sqrt(3.0)
        lv_kv_ln = basekv_lv / math.sqrt(3.0)
        ...
        tx_dss_lines = []
        for _node in tx_phase_nodes:
            unit_kva = tx_unit_kva.get(_node, rate_kva / len(tx_phase_nodes))
            u_core_kW, u_cu_kW, _ = lookup_tx_losses_kW(unit_kva)
            u_pct_noload = 100.0 * (u_core_kW / unit_kva) if unit_kva > 0 else 0.0
            u_pct_r = 100.0 * (u_cu_kW / (2.0 * unit_kva)) if unit_kva > 0 else 0.0
            tx_dss_lines.append(
                f"New Transformer.{tx_name}_{_letter[_node]} Phases=1 Windings=2 Xhl=2.72 conns=(wye, wye) %noloadloss={u_pct_noload:.6f}"
            )
            tx_dss_lines.append(
                f"~ wdg=1 bus=SourceBus.{_node}.0 conn=wye kv={hv_kv_ln:.6f} kva={unit_kva:.2f} %r={u_pct_r:.6f}"
            )
            tx_dss_lines.append(
                f"~ wdg=2 bus=source.{_node}.0 conn=wye kv={lv_kv_ln:.6f} kva={unit_kva:.2f} %r={u_pct_r:.6f}"
            )
        ...
        f.write("\n".join([
            ...
            *load_cmds,
            "",
             ...
             "CalcVoltageBases",
             "Solve",
            "! --- Bus XY coordinates (สำหรับ voltage map, ต้องอยู่หลัง Solve) ---",
            *busxy_cmds,
...
def solve_with_opendss(dss_file: str):
    # "Compile" ทำให้ OpenDSS chdir ไปโฟลเดอร์ของไฟล์ .dss เอง (ไม่คืนค่าเอง) —
    # ถ้าโฟลเดอร์นั้นเป็น temp dir ที่ผู้เรียกลบทิ้งหลัง solve (เช่น PhaseOptimizer._simulate
    # ที่เรียกซ้ำหลายรอบ) cwd ของ process จะค้างอยู่ในโฟลเดอร์ที่ถูกลบไปแล้ว ทำให้เรียก
    # solve_with_opendss ครั้งถัดไป crash (พบเฉพาะ macOS/Linux เพราะ Windows ล็อกไม่ให้ลบ
    # โฟลเดอร์ที่เป็น cwd อยู่ — บั๊กนี้ไม่เคยแสดงผลบน Windows)
    orig_cwd = os.getcwd()
    try:
        dss.Basic.ClearAll()
...
def report_transformer_pattern():
    ...
    # ต้องรวมผลจากทุกก้อนเข้าด้วยกัน — ถ้ามีก้อนเดียว (เคส 3 เฟสเต็ม) ผลจะเหมือนของเดิมทุกประการ
    hv_bus = lv_bus = None
    P_by_node: Dict[int, float] = {}
    Q_by_node: Dict[int, float] = {}
    I_by_node: Dict[int, float] = {}
    loss_kw = 0.0
    loss_kvar = 0.0
    tx_label_names = []

    for _name in tx_names:
        tx = f"Transformer.{_name}"
        tx_label_names.append(tx)
        ...
        for node, (p, q) in tx_secondary_power_by_node(tx).items():
            P_by_node[node] = P_by_node.get(node, 0.0) + p
            Q_by_node[node] = Q_by_node.get(node, 0.0) + q

        Id = element_terminal_current_dict(tx)
        for node, i in Id.get(2, {}).items():  # terminal 2 = secondary
            I_by_node[node] = I_by_node.get(node, 0.0) + i

        lw, lq = element_losses_kw(tx)
        loss_kw += lw
        loss_kvar += lq
    ...
    PA, QA = P_by_node.get(1, 0.0), Q_by_node.get(1, 0.0)
    PB, QB = P_by_node.get(2, 0.0), Q_by_node.get(2, 0.0)
    PC, QC = P_by_node.get(3, 0.0), Q_by_node.get(3, 0.0)
    P2 = PA + PB + PC
    Q2 = QA + QB + QC

    # Current A,B,C -> None ถ้าเฟสนั้นไม่มีอยู่จริง (ไม่ใช้เฟสปลอมจาก transformer เต็ม)
    IA = I_by_node.get(1)
    IB = I_by_node.get(2)
    IC = I_by_node.get(3)
    ...
    return {
        "TransformerName": " + ".join(tx_label_names),
        "SecondaryBus": lv_bus,
...
def process_one_json(json_path: str, output_root: Optional[str] = None, workdir: Optional[str] = None):
    ...
    dss_output_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "dssoutput")
...
def main():
    ...
    if not json_path and not json_dir:
        json_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "jsonfile")
