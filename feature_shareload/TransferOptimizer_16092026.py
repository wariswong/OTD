...
Options:
    ...
    --max-phase-loading  float  โหลดรายเฟสสูงสุดของ TR_A (% ของ PHASEx_KVA) [default 100]
    --max-b-phase-loading float โหลดรายเฟสสูงสุดของ TR_B หลังรับโหลด (%) [default = --load-limit]
...
from Runopendss_All16092026 import (
    ...
    solve_with_opendss,
    build_bfs_order,
    select_load_point_indices,
    select_service_line_indices,
    match_service_lines_to_meters,
    phase_node_from_letter,
    phase_nodes_from_designation,
)
...
class NetworkGraph:
    ...
    def __init__(self, json_path: str, snap_tol: float = 2.0,
        ...
        self.svc_match = svc_match      # ต้องเท่ากับ svc_match ของ convert_json_to_dss_ordered
        ...
        # มิเตอร์ → โหนดที่ต่อจริงทุกจุด (สายบริการ 1 เส้น = 1 จุด); load_feat_nodes = จุดแรก
        self.load_feat_conn_nodes: Dict[int, List[int]] = {}
    ...
    def _parse(self) -> None:
        ...
        # --- จุดต่อมิเตอร์: กฎเดียวกับ Runopendss (ฟังก์ชันร่วม) ---
        # มิเตอร์ต่อกับโครงข่ายผ่าน "สายบริการ" ที่โหนดใกล้ปลายสายบริการฝั่งสาย LV
        # (เดิมใช้โหนดใกล้ตัวมิเตอร์ → 12.7% ของมิเตอร์ต่อคนละโหนดกับ OpenDSS และบางตัวได้เฟสผิด)
        lp_idx  = select_load_point_indices(features)
        svc_idx = select_service_line_indices(features)
        matches = match_service_lines_to_meters(
            [point_xy(features[i]) for i in lp_idx],
            [features[i] for i in svc_idx], reps, self.svc_match)
        conn: Dict[int, List[int]] = defaultdict(list)
        for j, mi, nn in matches:
            conn[lp_idx[mi]].append(nn)
            self.svc_feat_nodes[svc_idx[j]] = nn          # โหนดฝั่งสาย LV ของสายบริการ
        for i in lp_idx:
            f = features[i]
            nodes = conn.get(i) or [nearest_node(point_xy(f))]   # ไม่มีสายบริการ → โหนดใกล้สุด
            nid = nodes[0]
            self.load_feat_nodes[i] = nid
            self.load_feat_conn_nodes[i] = nodes
            kw = float(get_attr(f, "KWP", 0.0) or 0.0)
            self.node_kw[nid] = self.node_kw.get(nid, 0.0) + kw
        # สายบริการที่จับคู่มิเตอร์ไม่ได้ (DSS ไม่ใช้) — เก็บไว้ให้ย้ายตามกิ่งใน TransferOptimizer เหมือนเดิม
        for i in svc_idx:
            if i not in self.svc_feat_nodes:
                ep = endpoints_from_paths(features[i])
                if ep:
                    self.svc_feat_nodes[i] = nearest_node(ep[0])
...
# =====================================================================
# การย้ายกิ่ง TR_A → TR_B: ใช้ร่วมกันระหว่าง _simulate_scenario และ collect_optimal_node_voltages
# =====================================================================

import inspect as _inspect

# pf ของโหลดใน DSS — อ่านจาก default ของ convert_json_to_dss_ordered (แปลง kW → kVA ในขั้นประมาณการ)
_LOAD_PF = float(_inspect.signature(convert_json_to_dss_ordered).parameters["default_pf"].default)
ESTIMATE_MARGIN_PCT = 10.0             # กรองประมาณการหลวมกว่าเกณฑ์จริง — ผ่าน/ไม่ผ่านให้ผลจำลองตัดสิน
DEFAULT_MAX_PHASE_LOADING_PCT = 100.0  # โหลดรายเฟสสูงสุดของหม้อแปลง (% ของ PHASEx_KVA)
_PHASE_KVA_FIELDS = {1: "PHASEA_KVA", 2: "PHASEB_KVA", 3: "PHASEC_KVA"}
_PD_BITS = (4, 2, 1)                   # A, B, C
_NODE_TO_PD_T = {1: 4, 2: 2, 3: 1}
_PD_TO_LETTER_T = {4: "A", 2: "B", 1: "C"}
_PD_LABEL_T = {7: "ABC", 6: "AB", 5: "CA", 4: "A", 3: "BC", 2: "B", 1: "C"}


def tx_phase_rating_from_raw(raw: dict) -> Dict[int, float]:
...
def query_transformer_phase_loading(
    phase_rating_kva: Optional[Dict[int, float]] = None,
) -> Tuple[float, Dict[int, float], Dict[int, float]]:
    """
    โหลดรายเฟสที่ขั้วทุติยภูมิของหม้อแปลงทุกก้อนในวงจรที่ solve แล้ว
    Returns (max % ของพิกัดเฟส, {node: %}, {node: kW})
    พิกัด: PHASEx_KVA จาก JSON เป็นหลัก, เฟสที่ไม่มีค่าใช้ kVA ขดลวด / จำนวนเฟส จาก DSS
    """
    kva: Dict[int, float] = {}
    kw: Dict[int, float] = {}
    rating: Dict[int, float] = {}
    for name in odss.Transformers.AllNames():
        ...
        for k in range(ncond):
            ...
            if node == 0:
                continue
            kva[node] = kva.get(node, 0.0) + math.hypot(pq[idx][0], pq[idx][1])
            kw[node] = kw.get(node, 0.0) + abs(pq[idx][0])
            rating[node] = rating.get(node, 0.0) + unit_kva / nph
    if phase_rating_kva:
        for n, rv in phase_rating_kva.items():
            if rv > 0 and n in rating:
                rating[n] = rv
    loading = {n: 100.0 * kva.get(n, 0.0) / r for n, r in rating.items() if r > 0}
    return max(loading.values(), default=0.0), loading, kw


def count_dead_loads() -> Tuple[int, float]:
    """โหลดที่ไม่ได้รับไฟในวงจรที่ solve แล้ว (kW > 0.01 แต่กินจริง < 5%) → (จำนวน, kW)"""
    n = 0
    kw_dead = 0.0
    i = odss.Loads.First()
    while i:
        kw = odss.Loads.kW()
        p = sum(odss.CktElement.Powers()[0::2])
        if kw > 0.01 and p < 0.05 * kw:
            n += 1
            kw_dead += kw
        i = odss.Loads.Next()
    return n, kw_dead
...
def _meter_pd_as_dss(feat: dict) -> int:
    """เฟสที่ Runopendss ต่อมิเตอร์จริง (PHASE ก่อน แล้วค่อยเฟสแรกของ PHASEDESIGNATION) → PD bit"""
    pnode = phase_node_from_letter(get_attr(feat, "PHASE"))
    if pnode is None:
        phs = phase_nodes_from_designation(get_attr(feat, "PHASEDESIGNATION"))
        pnode = phs[0] if phs else 1
    return _NODE_TO_PD_T.get(int(pnode), 4)


def _bits(pd: int) -> List[int]:
    return [b for b in _PD_BITS if pd & b]


def _pick_tie_phases(na_pd: int, nb_pd: int, b_load: Dict[int, float],
                     branch_load: Dict[int, float]) -> Tuple[int, Dict[int, int]]:
    """
    เลือกว่าจะลงเฟสไหนของ TR_B และจับคู่เฟสของกิ่ง → เฟสของ B

    การต่อ tie ไม่ต้องให้ป้ายเฟสสองฝั่งตรงกัน (ย้ายมาแล้วกิ่งก็อยู่บนเฟสที่ต่อเข้า)
    สิ่งที่ต้องดูคือโหลดต่อเฟส: ใช้เฟสของ B เท่าที่กิ่งต้องการ โดยเลือกเฟสที่เบาที่สุดก่อน
    แล้วจับคู่ "เฟสกิ่งที่หนักสุด → เฟส B ที่เบาสุด" เพื่อให้ B สมดุลที่สุด

    Returns (tie_pd, pmap) — pmap: เฟสกิ่ง → เฟส B (เฟสกิ่งที่ล้น = ไม่อยู่ใน pmap)
    """
    nb_bits = _bits(nb_pd) or list(_PD_BITS)
    na_bits = _bits(na_pd) or [nb_bits[0]]
    k = min(len(na_bits), len(nb_bits))
    chosen = sorted(nb_bits, key=lambda b: (b_load.get(b, 0.0), b))[:k]      # เฟส B ที่เบาสุด k เฟส
    src = sorted(na_bits, key=lambda b: (-branch_load.get(b, 0.0), b))[:k]   # เฟสกิ่งที่หนักสุด k เฟส
    dst = sorted(chosen, key=lambda b: (b_load.get(b, 0.0), b))
    pmap = {sb: db for sb, db in zip(src, dst)}
    return sum(chosen), pmap


def _fit_pd(orig: int, allowed: int) -> int:
    """เฟสของสายในกิ่งหลังย้าย: ใช้เฉพาะเฟสที่สาย tie มี; ไม่มีเฟสร่วมเลย → ย้ายไปเฟสของ tie
    โดยคงจำนวนตัวนำเดิม (สายเฟสเดียวยังเป็นเฟสเดียว)"""
    common = orig & allowed
    if common:
        return common
    k = max(bin(orig).count("1"), 1)
    bits = [b for b in _PD_BITS if allowed & b][:k]
    return sum(bits) or allowed


def _set_feature_phase(feat: dict, pd: int, meter: bool) -> dict:
    """สำเนาฟีเจอร์ที่ตั้ง PHASEDESIGNATION ใหม่ (มิเตอร์ตั้ง PHASE ด้วย เพราะ Runopendss อ่าน PHASE ก่อน)"""
    import copy
    f = copy.deepcopy(feat)
    attrs = f.setdefault("attributes", {})
    attrs["PHASEDESIGNATION"] = pd
    if meter:
        attrs["PHASE"] = _PD_TO_LETTER_T.get(pd, "A")
    return f


def _node_phase_in_subtree(net_a: "NetworkGraph", raw_a: dict, node: int, subtree: Set[int]) -> int:
    """เฟสที่มีจริงที่โหนดในกิ่ง = union เฟสของสายในกิ่งที่แตะโหนดนี้ (ไม่มีเลย → สายที่ป้อนโหนดเดิม)"""
    pd = 0
    for fi, (x, y) in net_a.lc_feat_edges.items():
        if x != y and (x == node or y == node) and x in subtree and y in subtree:
            pd |= int(get_attr(raw_a["features"][fi], "PHASEDESIGNATION", 7) or 7)
    return pd or _tie_phase(net_a, raw_a, node)


def build_transfer_features(
    net_a: "NetworkGraph", raw_a: dict,
    net_b: "NetworkGraph", raw_b: dict,
    scenario: dict,
) -> dict:
    """
    สร้างข้อมูลสำหรับจำลองการย้ายกิ่งจาก TR_A ไป TR_B
      1. สาย tie ใช้ทุกเฟสที่มีทั้งที่ n_b (ฝั่ง B) และที่ n_a (ในกิ่ง)
      2. สายในกิ่งใช้เฉพาะเฟสที่สาย tie มี (ไม่มีเฟสร่วม → ย้ายไปเฟสของ tie, จำนวนตัวนำเท่าเดิม)
      3. มิเตอร์ที่เฟสเดิมไม่มีไฟที่จุดต่อหลังย้าย → ย้ายไปเฟสที่มีไฟ
         (โหลดเยอะสุดก่อน ลงเฟสที่ TR_B เบาที่สุด) และบันทึกเป็นรายการที่ต้องย้ายเฟสหน้างาน
    Returns {remove_from_a, transferred, tie_pd, tie_sz, meter_moves[(PEANO, จาก, ไป, kW)]}
    """
    subtree = scenario["subtree_nodes"]
    n_a, n_b = scenario["tie_node_a"], scenario["tie_node_b"]
    sub_idx = net_a.get_feature_indices_for_subtree(subtree)
    remove_from_a: Set[int] = set(sub_idx)
    sw_feat = net_a.find_switch_edge_feature(scenario["switch_edge"])
    if sw_feat is not None:
        remove_from_a.add(sw_feat)

    na_pd = _node_phase_in_subtree(net_a, raw_a, n_a, subtree)
    nb_pd = _nb_phase(net_b, raw_b, n_b)
    # โหลดต่อเฟสของ TR_B (baseline) และของกิ่งที่จะโอน — ใช้เลือกเฟสที่จะลง
    b_load = {b: 0.0 for b in _PD_BITS}
    for node, kw in (scenario.get("b_phase_kw_base") or {}).items():
        bit = _NODE_TO_PD_T.get(int(node))
        if bit:
            b_load[bit] += float(kw)
    branch_load = {b: 0.0 for b in _PD_BITS}
    for fi in sub_idx:
        if fi in net_a.load_feat_nodes:
            branch_load[_meter_pd_as_dss(raw_a["features"][fi])] += float(
                get_attr(raw_a["features"][fi], "KWP", 0.0) or 0.0)
    tie_pd, pmap = _pick_tie_phases(na_pd, nb_pd, b_load, branch_load)
    xa, ya = net_a.node_coords[n_a]
    xb, yb = net_b.node_coords[n_b]
    tie_len = scenario.get("tie_distance_m")
    if tie_len is None:
        tie_len = math.hypot(xa - xb, ya - yb)
    tie_sz = _tie_conductor_size(net_a, raw_a, n_a, net_b, raw_b, n_b)
    # TR_A end offset → cluster_points ไม่รวมโหนด A กับ B ที่อยู่ใกล้กันจริง
    transferred: List[dict] = [make_tie_line_feature(
        xb, yb, xa + _COORD_OFFSET, ya + _COORD_OFFSET,
        length_m_override=tie_len, phase_designation=tie_pd, conductor_size=tie_sz)]

    new_pd: Dict[int, int] = {}
    meter_fis: List[int] = []
    for fi in sub_idx:
        if fi in net_a.lc_feat_edges:
            orig = int(get_attr(raw_a["features"][fi], "PHASEDESIGNATION", 7) or 7)
            pd = 0
            for b in _bits(orig):                     # map เฟสของสายทั้งชุดไปเฟสที่เลือก
                pd |= pmap.get(b, 0)
            if pd == 0:
                pd = _fit_pd(orig, tie_pd)
            new_pd[fi] = pd
            feat = raw_a["features"][fi] if pd == orig else _set_feature_phase(raw_a["features"][fi], pd, False)
            transferred.append(_translate_feature(feat, _COORD_OFFSET, _COORD_OFFSET))
        elif fi in net_a.load_feat_nodes:
            meter_fis.append(fi)
        else:                                   # สายบริการ — ใช้เฟสตามมิเตอร์อยู่แล้ว
            transferred.append(_translate_feature(raw_a["features"][fi], _COORD_OFFSET, _COORD_OFFSET))

    # เฟสที่มีไฟจริงที่แต่ละโหนด: ไล่จาก n_a (จุดรับไฟใหม่) ผ่านสายในกิ่ง
    adj: Dict[int, List[Tuple[int, int]]] = defaultdict(list)
    for fi, pd in new_pd.items():
        x, y = net_a.lc_feat_edges[fi]
        if x != y:
            adj[x].append((y, pd))
            adj[y].append((x, pd))
    live: Dict[int, int] = {n_a: tie_pd}
    dq = deque([n_a])
    while dq:
        x = dq.popleft()
        for y, pd in adj[x]:
            ph = live[x] & pd
            if ph & ~live.get(y, 0):
                live[y] = live.get(y, 0) | ph
                dq.append(y)

    used = dict(b_load)                        # ภาระต่อเฟสของ TR_B + กิ่งที่ลงเฟสนั้นแล้ว
    pend = []
    relabelled: List[Tuple[str, str, str, float]] = []
    moved: Dict[int, dict] = {}
    for fi in meter_fis:
        f = raw_a["features"][fi]
        kw = float(get_attr(f, "KWP", 0.0) or 0.0)
        cur = _meter_pd_as_dss(f)
        conns = getattr(net_a, "load_feat_conn_nodes", {}).get(fi) or [net_a.load_feat_nodes[fi]]
        avail = tie_pd
        for cn in conns:
            if cn in subtree:
                avail &= live.get(cn, 0)
        to = pmap.get(cur, 0)
        if to and (to & avail or not avail):
            # เฟสของมิเตอร์ตามเฟสของกิ่งที่ถูกต่อเข้าเฟสใหม่ — เปลี่ยนทั้งกิ่งที่จุด tie (งานจุดเดียว)
            used[to] += kw
            if to != cur:
                moved[fi] = _set_feature_phase(f, to, True)
                relabelled.append((str(get_attr(f, "PEANO", "") or "").strip(),
                                   _PD_TO_LETTER_T[cur], _PD_TO_LETTER_T[to], kw))
        else:
            pend.append((fi, cur, kw, avail))       # กิ่งมีเฟสมากกว่าจุด tie → ต้องแยกมิเตอร์จริง
    moves: List[Tuple[str, str, str, float]] = []
    for fi, cur, kw, avail in sorted(pend, key=lambda t: -t[2]):
        opts = [b for b in _PD_BITS if avail & b] or [b for b in _PD_BITS if tie_pd & b]
        to = min(opts, key=lambda b: used[b])
        used[to] += kw
        moved[fi] = _set_feature_phase(raw_a["features"][fi], to, True)
        moves.append((str(get_attr(raw_a["features"][fi], "PEANO", "") or "").strip(),
                      _PD_TO_LETTER_T[cur], _PD_TO_LETTER_T[to], kw))
    for fi in meter_fis:
        transferred.append(_translate_feature(moved.get(fi, raw_a["features"][fi]),
                                              _COORD_OFFSET, _COORD_OFFSET))
    pmap_txt = ", ".join(f"{_PD_TO_LETTER_T[a]}→{_PD_TO_LETTER_T[b]}" for a, b in sorted(pmap.items())
                         if a != b) or "ไม่เปลี่ยนเฟส"
    return {"remove_from_a": remove_from_a, "transferred": transferred,
            "tie_pd": tie_pd, "tie_sz": tie_sz, "meter_moves": moves,
            "relabelled": relabelled, "phase_map": pmap_txt}


def query_transformer_loading(rated_kva: float) -> float:
    """Return loading % = actual secondary kVA / rated_kva * 100 from active circuit.

    หม้อแปลง 1/2 เฟส จะถูกแยกเป็นหลาย Transformer object (คนละเฟส) ตอน generate DSS —
    ต้องรวม P,Q ของ secondary terminal จากทุกก้อน ไม่ใช่แค่ก้อนแรก
    """
    try:
        ...
        for name in tx_names:
            ...
            pq = list(zip(pq_flat[0::2], pq_flat[1::2]))
            # Secondary = last terminal; ncond ต่อ terminal ต่างกันตามจำนวนเฟสของหม้อแปลงก้อนนั้น
            # (3 เฟส+นิวทรัล = 4 conductors, 1 เฟส+นิวทรัล = 2 conductors) — อย่า hardcode 4
            ncond = odss.CktElement.NumConductors()
            n_phase_cond = max(ncond - 1, 1)  # ไม่นับ conductor นิวทรัล
            offset = len(pq) - ncond
            P2 += sum(pq[offset + i][0] for i in range(min(n_phase_cond, len(pq) - offset)))
            Q2 += sum(pq[offset + i][1] for i in range(min(n_phase_cond, len(pq) - offset)))
...
def collect_optimal_node_voltages(
    ...
    # ใช้ตัวสร้างเดียวกับ _simulate_scenario — แผนที่ตรงกับผลที่จำลองจริง (รวมการย้ายเฟสมิเตอร์)
    tf = build_transfer_features(net_a, raw_a, net_b, raw_b, opt)
    remove_from_a = tf["remove_from_a"]
    transferred = tf["transferred"]
...
class TransferOptimizer:
    def __init__(
        ...
        max_b_phase_loading_pct: Optional[float] = None,
        ...
        self.region = region
        self.max_phase_loading_pct = max_phase_loading_pct        # TR_A: ห้ามเฟสใดเกินพิกัด
        # TR_B: หลังรับโหลดแล้วทุกเฟสต้องมีที่ว่างเหลือเท่ากับเกณฑ์โหลดรวม (default = load_limit_pct)
        self.max_b_phase_loading_pct = (load_limit_pct if max_b_phase_loading_pct is None
                                        else max_b_phase_loading_pct)
        ...
        self.a_baseline_phase_max: float = 0.0
        self.b_baseline_phase_max: float = 0.0
        self._rating_a: Dict[int, float] = {}
        self._rating_b: Dict[int, float] = {}
        self._base_dead_a: int = 0
        self._base_dead_b: int = 0
    ...
    def _run_baseline_sim(self, json_path: Path, rated_kva: float, label: str,
                          snap_tol: float = 2.0, rating: Optional[Dict[int, float]] = None):
        """Run OpenDSS baseline for one network.
        Returns (loading_pct, min_voltage_v, max_phase_loading_pct, dead_loads, phase_kw)."""
        ...
        try:
            ...
            ph_max, _, ph_kw = query_transformer_phase_loading(rating)
            dead, _ = count_dead_loads()
            return loading, min_v, ph_max, dead, ph_kw
        finally:
            shutil.rmtree(tmp, ignore_errors=True)
    ...
    def _simulate_scenario(
        ...
        tf = build_transfer_features(self.net_a, raw_a, self.net_b, raw_b, scenario)
        remove_from_a = tf["remove_from_a"]
        transferred_features = tf["transferred"]
        tie_sz = tf["tie_sz"]

        tmp = tempfile.mkdtemp(prefix="topt_scen_")
        result = dict(scenario)
        result["tie_conductor_size"] = tie_sz
        result["tie_pd"] = tf["tie_pd"]
        result["tie_phase"] = _PD_LABEL_T.get(tf["tie_pd"], str(tf["tie_pd"]))
        result["meter_moves"] = tf["meter_moves"]          # ต้องแยกมิเตอร์จริง (งานรายมิเตอร์)
        result["n_meter_moves"] = len(tf["meter_moves"])
        result["relabelled"] = tf["relabelled"]            # เปลี่ยนเฟสทั้งกิ่งที่จุด tie (งานจุดเดียว)
        result["n_relabelled"] = len(tf["relabelled"])
        result["phase_map"] = tf["phase_map"]
        ...
        try:
            ...
            try:
                ...
                a_phase_max, _, _ = query_transformer_phase_loading(self._rating_a)
                a_dead, a_dead_kw = count_dead_loads()
            ...
            try:
                ...
                b_phase_max, _, _ = query_transformer_phase_loading(self._rating_b)
                b_dead, b_dead_kw = count_dead_loads()
            ...
            result["a_phase_max"] = a_phase_max
            result["b_phase_max"] = b_phase_max
            dead_new = (a_dead + b_dead) - (self._base_dead_a + self._base_dead_b)
            result["dead_loads_new"] = dead_new
            result["dead_kw"] = a_dead_kw + b_dead_kw
            reasons = []
            if a_loading >= self.load_limit_pct:
                reasons.append(f"TR_A {a_loading:.1f}%")
            if b_loading >= self.load_limit_pct:
                reasons.append(f"TR_B {b_loading:.1f}%")
            if result["min_v"] < self.min_voltage_v:
                reasons.append(f"Vmin {result['min_v']:.0f}V")
            if a_phase_max > self.max_phase_loading_pct:
                reasons.append(f"TR_A เฟสเกินพิกัด {a_phase_max:.0f}%")
            if b_phase_max > self.max_b_phase_loading_pct:
                reasons.append(f"TR_B เฟส {b_phase_max:.0f}% > {self.max_b_phase_loading_pct:.0f}%")
            if dead_new > 0:
                reasons.append(f"โหลดไม่ได้รับไฟเพิ่ม {dead_new} ราย")   # guard: ห้ามมีลูกค้าไฟดับ
            result["ng_reason"] = " | ".join(reasons)
    ...
    def run(self) -> List[dict]:
        ...
        print(f"  max_phase (TR_A/TR_B): {self.max_phase_loading_pct:.0f}% / "
              f"{self.max_b_phase_loading_pct:.0f}% ของพิกัดเฟส")
        print(f"{'='*60}")
        ...
        with open(self._json_b, encoding="utf-8") as fh:
            raw_b = json.load(fh)
        self._rating_a = tx_phase_rating_from_raw(raw_a)
        self._rating_b = tx_phase_rating_from_raw(raw_b)
        ...
        a_baseline, a_base_minv, a_ph, self._base_dead_a, _ = self._run_baseline_sim(
            self._json_a, self.net_a.rated_kva, "A", snap_tol=snap_tol_a, rating=self._rating_a)
        print(f"  TR_A: loading={a_baseline:.1f}%  เฟสสูงสุด={a_ph:.1f}%  V_min={a_base_minv:.1f}V")
        b_baseline, b_base_minv, b_ph, self._base_dead_b, b_phase_kw = self._run_baseline_sim(
            self._json_b, self.net_b.rated_kva, "B", snap_tol=snap_tol_b, rating=self._rating_b)
        print(f"  TR_B: loading={b_baseline:.1f}%  เฟสสูงสุด={b_ph:.1f}%  V_min={b_base_minv:.1f}V")
        self.a_baseline_phase_max, self.b_baseline_phase_max = a_ph, b_ph
        ...
        for (u, v) in list(self.net_a.G.edges()):
            ...
            # สายวงรอบ: ถ้ามีสายเส้นอื่น (นอกจากสวิตช์) เชื่อมกิ่งกับส่วนที่เหลือของ TR_A
            # การเปิดสวิตช์จุดเดียวจะทำให้หม้อแปลงสองลูกขนานกันจริง → ไม่รับ scenario นี้
            sw_fi = self.net_a.find_switch_edge_feature((u, v))
            if any(x != y and ((x in subtree) != (y in subtree)) and fi != sw_fi
                   for fi, (x, y) in self.net_a.lc_feat_edges.items()):
                n_loop_ng += 1
                continue
            ...
            # ประมาณการด้วย kVA (kW / pf ของโหลดใน DSS) และกรองหลวมกว่าเกณฑ์ ESTIMATE_MARGIN_PCT
            # — ใช้แค่ตัด scenario ที่ไม่มีทางผ่านแน่ ๆ ส่วนผ่าน/ไม่ผ่านให้ผลจำลองตัดสิน
            subtree_kva = subtree_kw / _LOAD_PF
            est_a_kva = max(0.0, (a_baseline / 100.0) * self.net_a.rated_kva - subtree_kva)
            est_b_kva = (b_baseline / 100.0) * self.net_b.rated_kva + subtree_kva
            est_a_pct = est_a_kva / self.net_a.rated_kva * 100.0
            est_b_pct = est_b_kva / self.net_b.rated_kva * 100.0
            est_lim = self.load_limit_pct + ESTIMATE_MARGIN_PCT
            if est_a_pct >= est_lim or est_b_pct >= est_lim:
                continue
        ...
        if n_loop_ng:
            print(f"  (ตัด {n_loop_ng} scenario ที่มีสายวงรอบเชื่อมกิ่งกับ TR_A — เปิดสวิตช์จุดเดียวไม่พอ)")
        if not candidates:
...
def print_results_table(results: List[dict], net_a: NetworkGraph, net_b: NetworkGraph) -> None:
    ...
    hdr = (f"{'#':>4} {'SW_U':>8} {'SW_V':>8} {'TieA':>8} {'TieB':>8} "
           ...
           f"{'B_bef%':>7} {'B_aft%':>7} {'PhMax%':>7} {'V_min(V)':>9} {'CndSz':>6} "
           f"{'Tie':>4} {'PhaseMap':>12} {'MvPh':>5} {'OK':>5}")
    print(hdr)
    print("-" * col)
    rank = 0
    for r in results:
        ...
        row = (f"{rank:>4} {su:>8} {sv:>8} {na:>8} {nb:>8} "
               ...
               f"{max(r.get('a_phase_max', 0), r.get('b_phase_max', 0)):>7.1f} "
               f"{r['min_v']:>9.1f} {csz:>4}mm2 {r.get('tie_phase', ''):>4} "
               f"{r.get('phase_map', ''):>12} {r.get('n_meter_moves', 0):>5} {mark:>5}")
        print(row)
    if rank == 0:
...
def save_excel_report(
    ...
    headers = [
        ...
        "TRA_PhaseMax%", "TRB_PhaseMax%", "Tie_Phase", "Phase_Map",
        "Meters_Split (งานรายมิเตอร์)", "Meters_Relabelled (เปลี่ยนทั้งกิ่ง)", "DeadLoads_New",
        "Feasible", "Error / เหตุที่ไม่ผ่าน",
    ]
    ws.append(headers)
    for cell in ws[1]:
        cell.fill = hdr_fill
    ...
    for r in results:
        ...
        row = [
            ...
            round(r.get("a_phase_max", 0), 1) if "a_phase_max" in r else "",
            round(r.get("b_phase_max", 0), 1) if "b_phase_max" in r else "",
            r.get("tie_phase", ""),
            r.get("phase_map", ""),
            r.get("n_meter_moves", ""),
            r.get("n_relabelled", ""),
            r.get("dead_loads_new", ""),
            "YES" if feasible else "NO",
            r.get("error") or r.get("ng_reason") or "",
        ]
        ws.append(row)
        ridx = ws.max_row
    ...
    if opt:
        rows_summary += [
            ...
            ["Tie Line Phase", opt.get("tie_phase", "")],
            ["TR_A Max Phase Loading (% of PHASEx_KVA)", round(opt.get("a_phase_max", 0), 1)],
            ["TR_B Max Phase Loading (% of PHASEx_KVA)", round(opt.get("b_phase_max", 0), 1)],
            ["Tie Phase Map (กิ่ง → TR_B)", opt.get("phase_map", "")],
            ["Meters to split (งานรายมิเตอร์ — ดูชีต Meter Phase Moves)", opt.get("n_meter_moves", 0)],
            ["Meters relabelled (เปลี่ยนเฟสทั้งกิ่งที่จุด tie)", opt.get("n_relabelled", 0)],
        ]
    for row in rows_summary:
        ws2.append(row)

    # ---- Sheet: Meter Phase Moves (optimal) ----
    ws3 = wb.create_sheet("Meter Phase Moves")
    ws3.append(["PEANO", "From Phase", "To Phase", "kW", "หมายเหตุ"])
    for cell in ws3[1]:
        cell.fill = hdr_fill
        cell.font = hdr_font
    if opt:
        for pe, frm, to, kw in opt.get("meter_moves", []):
            ws3.append([pe, frm, to, round(kw, 3), "ต้องย้ายเฟสมิเตอร์ตัวนี้เอง (กิ่งมีเฟสมากกว่าจุด tie)"])
        for pe, frm, to, kw in opt.get("relabelled", []):
            ws3.append([pe, frm, to, round(kw, 3), "เปลี่ยนตามเฟสของกิ่งที่จุด tie (ไม่ต้องทำที่มิเตอร์)"])
...
def main() -> None:
    ...
    ap.add_argument("--max-phase-loading", type=float, default=DEFAULT_MAX_PHASE_LOADING_PCT,
                    metavar="PCT", help="โหลดรายเฟสสูงสุดของ TR_A (%% ของ PHASEx_KVA)")
    ap.add_argument("--max-b-phase-loading", type=float, default=None, metavar="PCT",
                    help="โหลดรายเฟสสูงสุดของ TR_B หลังรับโหลด (%% ของ PHASEx_KVA; ไม่ระบุ = ใช้ --load-limit)")
    ...
    optimizer = TransferOptimizer(
        ...
        max_b_phase_loading_pct=args.max_b_phase_loading,
        region=args.region,
    )
