...
Usage:
    python PhaseOptimizer_03092026.py <FACILITYID> [options]
...
def simulate_phase_addition(
    ...
    """
    Upgrade non-3-phase LV line components to 3-phase (ABC) and redistribute
    ...
    imbalance_mode = (not low_v_set and
                      baseline.phase_imbalance_pct > max_imbalance_pct)

    if not low_v_set and not imbalance_mode:
        print("\n[Phase Addition] ไม่มีโหนดแรงดันต่ำ และ imbalance ไม่เกินเกณฑ์ — ข้าม")
        return []
    ...
    # ถ้ามีหลายกลุ่ม → เพิ่มตัวเลือกรวมทุกกลุ่มเป้าหมาย
    if len(target_cis) > 1:
...
class LVOptimizer:
    ...
    def run(self) -> None:
        ...
        def _pa_worth_applying(pa: PhaseAddResult) -> bool:
            ...
            if pa.delta_min_v > 0:
                return True
            return pa.delta_imbalance >= 3.0 and pa.delta_min_v >= -1.0

        if self.phase_add_options and _pa_worth_applying(self.phase_add_options[0]):
...
def draw_interactive_map(opt: LVOptimizer, out_path: str) -> None:
    ...
    for pa in opt.phase_add_options[:2]:
        ...
        rows_val.append([f"Phase Addition {_PD_LABEL.get(pa.from_pd,'?')}→ABC{marker}",
