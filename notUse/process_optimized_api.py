...
def get_transformer_metadata_from_json(json_input, facilityid):
    """
    Extracts transformer metadata (PEA_NO, RATEKVA, LOCATION, etc.) from the input JSON features.
    """
    data = None
    ...
    for f in features:
        ...
        if fid == str(facilityid).strip():
            return {
                "tr_pea_no": attrs.get("PEANO") or attrs.get("FACILITYID") or "",
                "tr_kva": float(attrs.get("RATEKVA", 0) or 0),
                "tr_location": attrs.get("LOCATION", ""),
                "tr_loss": attrs.get("LOSS", 1.0),
                "tr_Line_lengh": attrs.get("MAX_LEN", 1.0),
                "tr_PLOADPEAK": attrs.get("PLOAD_PEAK") or attrs.get("PLOAD_KW") or 0.0
            }
    return {}
...
def optimizeGroup(meterLocations, phase_loads, initialTransformerLocation,
    ...
    # ---- Prefer PHASEDESIGNATION=7 (ABC) from JSON indexer ----
    selected_phase_code = None
    if callable(lvData):
        allowed_for_xy = lvData

        def _is_abc(ph):
            s = set(str(x).upper() for x in (ph or []))
            return s == {"A", "B", "C"}  # ABC = 7

        cand_abc = []
        for n in candidates:
            xy = coord_mapping_temp.get(n)
            if xy is None:
                continue
            if _is_abc(allowed_for_xy(xy)):
                cand_abc.append(n)

        logging.info("Phase=7(ABC) candidate filter (JSON): %d -> %d", len(candidates), len(cand_abc))

        if cand_abc:
            candidates = cand_abc
            selected_phase_code = 7
        else:
            logging.warning("No candidates on PHASEDESIGNATION=7 (ABC) from JSON; fallback to all candidates.")
            selected_phase_code = None
    
    else:
        # (ออปชัน) ถ้าคุณยังอยากรองรับ shapefile ด้วย ค่อยคงของเดิมไว้ใน else
        phase_priority = (7, 6, 5, 4, 3, 2, 1)
        lv_phase_pts, selected_phase_code = _collect_lv_vertices_by_phase_priority(lvData, phase_priority=phase_priority)
        if lv_phase_pts:
            ...
            logging.info("Phase-priority candidate filter selected PHASEDESIGNATION=%s : %d -> %d nodes (tol=%.2f)",
                        selected_phase_code, cand_before, len(candidates_phase), tol_match)

            if candidates_phase:
                candidates = candidates_phase
            else:
                logging.warning("Selected phase code=%s but no graph nodes matched LV vertices. Using unfiltered candidates.",
                                selected_phase_code)
        else:
            logging.warning("No LV lines found for PHASEDESIGNATION in priority %s; using all candidates.", phase_priority)
...
def summarize_unbalance_change(original_phase_loads: dict, new_phase_loads: dict):
    ...
    logging.info(f"Delta %Unbalance = {pct_after - pct_before:+.2f} (negative = improved)")
...
def summarize_loss_change(
    ...
    *, conductorResistance: float, powerFactor: float, initialVoltage: float,
    ...
    logging.info(f"Delta Loss = {loss_after - loss_before:+.2f} W ({(loss_after - loss_before)/1000:+.3f} kW)")
...
# ---------------------------------
# 21) plotResults_NGUI, main_pipeline
def plotResults_NGUI(project_id, lvLinesX, lvLinesY, mvLinesX, mvLinesY, eserviceLinesX, eserviceLinesY,
    ...
    # Step 1: โปรเจกชัน (UTM Zone 47/48 ตามความเหมาะสม)
    # เราใช้ Zone 47 ตามมาตรฐานที่ลูกค้าระบุในสคริปต์ก่อนหน้า
    proj_tm3 = pyproj.CRS.from_proj4("+proj=utm +zone=47 +datum=WGS84 +units=m +no_defs")
    proj_wgs84 = pyproj.CRS("EPSG:4326")
    transformer_proj = pyproj.Transformer.from_crs(proj_tm3, proj_wgs84, always_xy=True)
    ...
    for x_list, y_list in zip(lvLinesX, lvLinesY):
        latlons = [transformer_proj.transform(x, y) for x, y in zip(x_list, y_list)]
        featuresLV.append({
            "type": "Feature",
            "geometry": {"type": "LineString", "coordinates": latlons},
            "properties": {"type": "LV Line"}
        })

    with open(os.path.join(output_dir, "lv_lines.geojson"), "w", encoding="utf-8") as f:
        json.dump({"type": "FeatureCollection", "features": featuresLV}, f, indent=2)    

    featuresMV = []
    for mvx_list, mvy_list in zip(mvLinesX, mvLinesY):
        latlons = [transformer_proj.transform(x, y) for x, y in zip(mvx_list, mvy_list)]
        featuresMV.append({
            "type": "Feature",
            "geometry": {"type": "LineString", "coordinates": latlons},
    ...
    with open(os.path.join(output_dir, "mv_lines.geojson"), "w", encoding="utf-8") as f:
        json.dump({"type": "FeatureCollection", "features": featuresMV}, f, indent=2)
    ...
    def get_voltage_text(i):
        ...
        for ph in ['A', 'B', 'C']:
            if ph in connected_phases:
                ...
                if colname in result_df.columns:
                    vval = result_df.iloc[i][colname]
                    if pd.notnull(vval): voltage_text += f'{ph}:{vval:.1f}V\n'
                    else: voltage_text += f'{ph}:N/A\n'
    ...
    for idx in group1_indices:
        ...
        features_meters.append({
            "type": "Feature",
            "geometry": {"type": "Point", "coordinates": [lon, lat]},
            "properties": {"group": 1, "voltage_text": get_voltage_text(idx)}
        })
    for idx in group2_indices:
        ...
        features_meters.append({
            "type": "Feature",
            "geometry": {"type": "Point", "coordinates": [lon, lat]},
            "properties": {"group": 2, "voltage_text": get_voltage_text(idx)}
        })

    with open(os.path.join(output_dir, "meter_groups.geojson"), "w", encoding="utf-8") as f:
        json.dump({"type": "FeatureCollection", "features": features_meters}, f, ensure_ascii=False, indent=2)

    features_pts = []
    if initialTransformerLocation is not None:
        lon, lat = transformer_proj.transform(initialTransformerLocation[0], initialTransformerLocation[1])
        features_pts.append(Feature(geometry=Point((lon, lat)), properties={"name": "Initial Transformer", "group": "initial"}))
    if splitting_point_coords is not None:
        lon, lat = transformer_proj.transform(splitting_point_coords[0], splitting_point_coords[1])
        features_pts.append(Feature(geometry=Point((lon, lat)), properties={"name": "Splitting Point", "group": "splitting"}))
    if optimizedTransformerLocationGroup1 is not None:
        lon, lat = transformer_proj.transform(optimizedTransformerLocationGroup1[0], optimizedTransformerLocationGroup1[1])
        features_pts.append(Feature(geometry=Point((lon, lat)), properties={"name": "Group 1 Transformer", "group": "group1"}))
    if optimizedTransformerLocationGroup2 is not None:
        lon, lat = transformer_proj.transform(optimizedTransformerLocationGroup2[0], optimizedTransformerLocationGroup2[1])
        features_pts.append(Feature(geometry=Point((lon, lat)), properties={"name": "Group 2 Transformer", "group": "group2"}))

    with open(os.path.join(output_dir, "feature_groups.geojson"), "w", encoding="utf-8") as f:
        json.dump(FeatureCollection(features_pts), f, ensure_ascii=False, indent=2)
    
    print("Export GeoJSON OK!")
...
def main_pipeline(project_id: str, facility_id: str, sp_index: int=0):
    ...
    global conductorReactance, lvData, svcLines, SNAP_TOLERANCE, IS_GUI
    ...
    # Note: Facility ID in filename might be prefixed differently, but usually it matches
    json_path = os.path.join(base_dir, f"{project_id}_NetworkLV{facility_id}_with_MV.json")

    if not os.path.exists(json_path):
        # Fallback search if exact name formula differs
        json_pattern = os.path.join(base_dir, f"*NetworkLV{facility_id}*.json")
        matching_files = glob.glob(json_pattern)
        if matching_files:
            json_path = matching_files[0]
            logging.info(f"Found matching JSON via pattern: {json_path}")
        else:
            raise FileNotFoundError(f"Input JSON not found for project={project_id}, fac={facility_id}")
    ...
    setup_logging(log_path, to_console=True, level=logging.INFO, tee_stdout_to_file=True)

    try:
        # 1) Extract Data
        meterLocations, initialVoltages, totalLoads, phase_loads, peano, phases = extractMeterData_json(json_path)
        lvLinesX, lvLinesY, lvLines, _snap_lv, _snap_map_lv = extractLineData_json(json_path)
        
        try:
            mvLinesX, mvLinesY, mvLines, _snap_mv, _snap_map_mv = extractMVLineData_json(json_path, tag_contains="MC")
        except:
            mvLinesX, mvLinesY, mvLines = [], [], []

        try:
            eserviceLinesX, eserviceLinesY, filteredEserviceLines, _snap_svc, _snap_map_svc = extractServiceLines_json(json_path)
        except:
            filteredEserviceLines = []

        length_map = build_line_length_map_from_json(json_path, coord_snap_map=_snap_map_lv)
        
        # 2) Coordinate Snapping & Tolerance
        SNAP_TOLERANCE = auto_determine_snap_tolerance(meterLocations, lvLines, mvLines, use_analysis=True)
        logging.info(f"Automatically determined SNAP_TOLERANCE: {SNAP_TOLERANCE:.8f} m")

        tx_xy = get_transformer_xy_from_json(json_path, facilityid=facility_id)
        transformerCapacity_kVA, transformerCapacity, powerFactor = get_transformer_capacity_from_json(json_path)
        ...
        # 3) Build Network & Initial Analysis
        all_line_coords = []
        ...
        G_init, tNode_init, mNodes_init, _, _ = buildLVNetworkWithLoads(
            lvLines, mvLines, meterLocations, tx_xy, phase_loads,
            conductorResistance, conductorReactance, svcLines=svcLines,
            use_shape_length=True, line_length_map=length_map, coord_snap_map=coord_snap_map
        )

        # 4) Optimize TR Location (Primary)
        optimizedTransformerLocation = optimizeTransformerLocationOnLVCond3(
            meterLocations, phase_loads, tx_xy, lvLines, initialVoltage,
            conductorResistance, powerFactor, conductorReactance=conductorReactance, svcLines=svcLines
        )
        
        # 5) Network Partitioning (Splitting)
        G, transformerNode, meterNodes, node_mapping, coord_mapping = buildLVNetworkWithLoads(
            lvLines, mvLines, meterLocations, optimizedTransformerLocation, phase_loads,
            conductorResistance, conductorReactance, svcLines=svcLines,
            use_shape_length=True, line_length_map=length_map, coord_snap_map=coord_snap_map
        )

        splitting_edge, sp_coord, sp_edge_diff, _ = findSplittingPoint(
            G, transformerNode, meterNodes, coord_mapping, powerFactor, initialVoltage, candidate_index=sp_index
        )

        if splitting_edge is None:
            logging.warning("No splitting edge found. Ending pipeline.")
            return

        # 6) Load Division and Phase Balancing
        voltages, _, group1_nodes, group2_nodes = performForwardBackwardSweepAndDivideLoads(
        ...
        phase_indexer = build_phase_indexer_from_json(json_path)
        
        def safe_balance(indices):
            if not indices: return [], {}
            return optimize_phase_balance(
                meterLocations[indices], totalLoads[indices], {k: v[indices] for k,v in phase_loads.items()},
                peano[indices], None, original_phases=phases[indices].tolist(), phase_indexer=phase_indexer
            )

        new_phases_g1, new_phase_loads_g1 = safe_balance(g1_idx)
        new_phases_g2, new_phase_loads_g2 = safe_balance(g2_idx)

        # 7) Per-Group Transformer Location Optimization
        def optimize_group_tr(indices, phase_ld):
            if not indices: return None
            lc = calculateNetworkLoadCenter(meterLocations[indices], phase_ld, lvLines, mvLines, conductorResistance)
            return optimizeGroup(meterLocations[indices], phase_ld, lc, lvLines, mvLines, initialVoltage, conductorResistance, powerFactor)

        optimizedTransformerLocationGroup1 = optimize_group_tr(g1_idx, new_phase_loads_g1)
        optimizedTransformerLocationGroup2 = optimize_group_tr(g2_idx, new_phase_loads_g2)

        # 8) Final Performance Metrics & Result DF
        dist_arr = []
        for n in meterNodes:
            try: dval = nx.shortest_path_length(G, transformerNode, n, weight='weight')
            except: dval = float('inf')
            dist_arr.append(dval)

        result_df = pd.DataFrame({
            'Peano Meter': peano,
            'Final Voltage A (V)': [voltages[n]['A'] for n in meterNodes],
            'Final Voltage B (V)': [voltages[n]['B'] for n in meterNodes],
            'Final Voltage C (V)': [voltages[n]['C'] for n in meterNodes],
            'Distance to Transformer (m)': dist_arr,
            'Load A (kW)': phase_loads['A'], 'Load B (kW)': phase_loads['B'], 'Load C (kW)': phase_loads['C'],
            'Group': ['Group 1' if n in group1_nodes else 'Group 2' for n in meterNodes],
            'Phases': phases, 'Meter X': meterLocations[:, 0], 'Meter Y': meterLocations[:, 1]
        })

        # 9) Export Outputs
        output_folder = os.path.join("pea_no_projects", "output", str(project_id))
        os.makedirs(output_folder, exist_ok=True)
        
        plotResults_NGUI(
            project_id, lvLinesX, lvLinesY, mvLinesX, mvLinesY, eserviceLinesX, eserviceLinesY,
            meterLocations, tx_xy, optimizedTransformerLocation, g1_idx, g2_idx,
            splitting_point_coords=sp_coord, result_df=result_df, phases=phases,
        ...
        def get_phase_sums(p_loads, idx=None):
            if idx is not None:
                return {
                    "A": round(float(p_loads['A'][idx].sum()), 1),
                    "B": round(float(p_loads['B'][idx].sum()), 1),
                    "C": round(float(p_loads['C'][idx].sum()), 1)
                }
            return {
                "A": round(float(p_loads['A'].sum()), 1),
                "B": round(float(p_loads['B'].sum()), 1),
                "C": round(float(p_loads['C'].sum()), 1)
            }

        # Extract metadata from JSON
        tr_meta = get_transformer_metadata_from_json(json_path, facility_id)

        # predicted loads
        g1_load_val = totalLoads[g1_idx].sum() if g1_idx else 0
        g2_load_val = totalLoads[g2_idx].sum() if g2_idx else 0
        future_g1_load = growthRate(g1_load_val, annual_growth=0.06, years=4)
        future_g2_load = growthRate(g2_load_val, annual_growth=0.06, years=4)
        rating_g1 = Lossdocument(future_g1_load)
        rating_g2 = Lossdocument(future_g2_load)

        # Loss summary per group
        tx_loc_g1 = optimizedTransformerLocationGroup1 if optimizedTransformerLocationGroup1 is not None else tx_xy
        loss_g1_summary = summarize_loss_change(
            lvLines, mvLines, meterLocations[g1_idx], tx_loc_g1,
            {k:v[g1_idx] for k,v in phase_loads.items()}, new_phase_loads_g1,
            conductorResistance=conductorResistance, powerFactor=powerFactor, initialVoltage=initialVoltage,
            conductorReactance=conductorReactance, lvData=json_path, svcLines=svcLines
        ) if g1_idx else None
        
        tx_loc_g2 = optimizedTransformerLocationGroup2 if optimizedTransformerLocationGroup2 is not None else tx_xy
        loss_g2_summary = summarize_loss_change(
            lvLines, mvLines, meterLocations[g2_idx], tx_loc_g2,
            {k:v[g2_idx] for k,v in phase_loads.items()}, new_phase_loads_g2,
            conductorResistance=conductorResistance, powerFactor=powerFactor, initialVoltage=initialVoltage,
            conductorReactance=conductorReactance, lvData=json_path, svcLines=svcLines
        ) if g2_idx else None

        # Predicted loads and TR losses
        tx_loss_g1 = get_transformer_losses(g1_load_val)
        tx_loss_g2 = get_transformer_losses(g2_load_val)
        
        # Unbalance metrics
        g1_unb_info = summarize_unbalance_change({k:v[g1_idx] for k,v in phase_loads.items()}, new_phase_loads_g1) if g1_idx else {"before":{"pct_unbalance":0},"after":{"pct_unbalance":0}}
        g2_unb_info = summarize_unbalance_change({k:v[g2_idx] for k,v in phase_loads.items()}, new_phase_loads_g2) if g2_idx else {"before":{"pct_unbalance":0},"after":{"pct_unbalance":0}}

        results = {
            "tr_pea_no": tr_meta.get("tr_pea_no", facility_id),
            "tr_kva": tr_meta.get("tr_kva", round(transformerCapacity_kVA, 2)),
            "tr_loss": tr_meta.get("tr_loss", 1.0),
            "tr_Line_lengh": tr_meta.get("tr_Line_lengh", 1.0),
            "tr_PLOADPEAK": tr_meta.get("tr_PLOADPEAK", g1_load_val + g2_load_val),

            "g1_load": round(g1_load_val, 2),
            "future_g1_load": round(future_g1_load, 2),
            "rating_g1": rating_g1,
            "line_loss_g1_kW": round(loss_g1_summary["loss_after_kW"], 2) if loss_g1_summary else 0.0,
            "tx_loss_g1_kW": round(tx_loss_g1, 2),
            "total_system_loss_g1": round((loss_g1_summary["loss_after_kW"] if loss_g1_summary else 0.0) + tx_loss_g1, 2),

            "g2_load": round(g2_load_val, 2),
            "future_g2_load": round(future_g2_load, 2),
            "rating_g2": rating_g2,
            "line_loss_g2_kW": round(loss_g2_summary["loss_after_kW"], 2) if loss_g2_summary else 0.0,
            "tx_loss_g2_kW": round(tx_loss_g2, 2),
            "total_system_loss_g2": round((loss_g2_summary["loss_after_kW"] if loss_g2_summary else 0.0) + tx_loss_g2, 2),

            "Group_1_percent_unbalance_before": round(g1_unb_info["before"]["pct_unbalance"], 1),
            "Group_1_percent_unbalance_after": round(g1_unb_info["after"]["pct_unbalance"], 1),
            "Group_2_percent_unbalance_before": round(g2_unb_info["before"]["pct_unbalance"], 1),
            "Group_2_percent_unbalance_after": round(g2_unb_info["after"]["pct_unbalance"], 1),

            "Max_Distance_Group1": round(max([dist_arr[i] for i in g1_idx]) if g1_idx else 0, 1),
            "Max_Distance_Group2": round(max([dist_arr[i] for i in g2_idx]) if g2_idx else 0, 1),

            "group1_load_balance_before": get_phase_sums(phase_loads, g1_idx),
            "group1_load_balance_after": get_phase_sums(new_phase_loads_g1),
            "group2_load_balance_before": get_phase_sums(phase_loads, g2_idx),
            "group2_load_balance_after": get_phase_sums(new_phase_loads_g2)
        }
        
        with open(os.path.join(output_folder, "results.json"), "w", encoding="utf-8") as f:
            json.dump(results, f, ensure_ascii=False, indent=2)

        logging.info("[PIPELINE] Finished successfully. Results saved to results.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Headless Transformer Optimization Pipeline")
    parser.add_argument("--project_id", required=True, help="Project ID")
    parser.add_argument("--facility_id", required=True, help="Facility ID")
    parser.add_argument("--sp_index", type=int, default=0, help="Splitting point index")
    args = parser.parse_args()

    try:
        main_pipeline(args.project_id, args.facility_id, args.sp_index)
        sys.exit(0)
    except Exception as e:
        print(f"Error: {e}")
        sys.exit(1)
