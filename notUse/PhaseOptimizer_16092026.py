...
Usage:
    python PhaseOptimizer_16092026.py <FACILITYID> [options]
...
def simulate_phase_addition(
    ...
    for feat_idx, (pu, pv) in net.lc_feat_edges.items():
        ...
        if (pd & tx_mask) == tx_mask:
            continue  # มีครบทุกเฟสของหม้อแปลงแล้ว ไม่ต้องเพิ่ม
    ...
    def _run_comp(comp_nodes, comp_edges, label):
        ...
        for fi, kw in sorted(meters_here, key=lambda x: -x[1]):   # โหลดเยอะสุดก่อน
            best_pd = min(tx_pds, key=lambda p: phase_kw_used[p])
