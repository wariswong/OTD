...
def build_meter_bus_names(features: List[Dict[str, Any]]) -> Dict[int, str]:
    ...
    suffixing exactly (convert_json_to_dss_ordered applies the same rules
    inline when building load_points/meter_meta), so other code — e.g.
    voltage lookups matching a meter back to its simulated bus — has one
    source of truth instead of re-deriving the naming/dedup rules and
    risking silent drift from what actually got written to the .dss file.
    """
    names: Dict[int, str] = {}
...
def convert_json_to_dss_ordered(
    ...
    if tx_is_full3ph:
        ...
        # เลือก node ที่มิเตอร์จริงต่ออยู่มากที่สุดเป็น secondary node เดียวของหม้อแปลง
