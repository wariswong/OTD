...
recsplit_btn = None # ปุ่ม "Split Subgroups (Recursive)" — กัน NameError ตอน headless
_LOG_CONFIGURED = False
out_json = None  # set by __main__ before calling main(), so main() picks the right JSON
OUT_DIR = './testpy'  # per-run output folder; overridden in main() based on FACILITYID
_API_PROJECT_ID = None  # set by main_pipeline() for API/headless mode
_API_SP_INDEX = 0      # splitting candidate index for API/headless mode
RECURSIVE_SPLIT_ENABLED = False   # เปิดด้วย --recursive-split
RECURSIVE_SPLIT_MAX_KVA = 250.0   # หยุดแบ่งเมื่อโหลดกลุ่ม <= ค่านี้ (kVA)
RECURSIVE_SPLIT_MAX_DEPTH = 5     # safety valve กันแบ่งลึกเกินไปในกราฟผิดปกติ
...
def snap_coordinates_to_tolerance(coords_list, tolerance=0.1):
    ...
    ใช้ Union-Find เพื่อรับประกัน transitive closure (A≈B, B≈C → A,B,C กลุ่มเดียวกัน)
    ...
    parent = list(range(len(coords_list)))
    ...
    # For each group, pick the smallest-index member as representative
    group_rep = {}
    for i in range(len(coords_list)):
        root = find(i)
        if root not in group_rep:
            group_rep[root] = i

    coord_mapping = {}
    for i, coord in enumerate(coords_list):
        rep_idx = group_rep[find(i)]
        coord_mapping[coord] = coords_list[rep_idx]
...
def buildLVNetworkWithLoads(
    ...
    # ---------- ถ้า transformer อยู่ component ที่มี meter น้อย ให้เชื่อมไป component หลักของ meter ----------
    try:
        if G.number_of_nodes() > 0 and meterNodes:
            comps_tmp = list(nx.connected_components(G))
            meter_set_tmp = set(meterNodes)
            tx_comp = next((c for c in comps_tmp if transformerNode in c), set())
            tx_meter_count = len(meter_set_tmp.intersection(tx_comp))
            comp_meter_counts = [(len(meter_set_tmp.intersection(c)), c) for c in comps_tmp]
            best_count, best_comp = max(comp_meter_counts, key=lambda x: x[0]) if comp_meter_counts else (0, set())
            if best_count > max(tx_meter_count, 1):
                tx_xy = np.array(coord_mapping[transformerNode], dtype=float)
                best_lv_nodes = [n for n in best_comp if n in coord_mapping and n not in meter_set_tmp]
                if best_lv_nodes:
                    pts = np.array([coord_mapping[n] for n in best_lv_nodes], dtype=float)
                    dists = np.sqrt(np.sum((pts - tx_xy) ** 2, axis=1))
                    idx = int(np.argmin(dists))
                    target = best_lv_nodes[idx]
                    dist = float(dists[idx])
                    Rt = (dist / 1000.0) * conductorResistance
                    Xt = (dist / 1000.0) * (conductorReactance if conductorReactance is not None else 0.1 * conductorResistance)
                    G.add_edge(transformerNode, target, weight=dist, resistance=Rt, reactance=Xt, is_service=False, tx_component_reconnect=True)
                    logging.warning("Transformer component had %d meters, largest component had %d meters; added reconnect edge to node %s at %.2f m.", tx_meter_count, best_count, target, dist)
    except Exception as e:
        logging.warning(f"Transformer component reconnect check skipped: {e}")
...
def calculateNetworkLoadCenter(meterLocations, phase_loads, lvLines, mvLines, conductorResistance,
    ...
    # Bridge disconnected meter components to the main component so every meter
    # contributes to the distance sums.  A meter isolated in a small component
    # would otherwise have score≈0 (distance to itself = 0) and falsely win the
    # argmin even though only 1 meter can "see" it.
    comps_check = list(nx.connected_components(G))
    if len(comps_check) > 1:
        meter_set_check = set(mNodes)
        comp_meter_cnt  = [(len(meter_set_check.intersection(c)), c) for c in comps_check]
        main_cnt, main_comp = max(comp_meter_cnt, key=lambda x: x[0])
        main_lv_nodes = [n for n in main_comp
                         if n in coord_mapping and n not in meter_set_check]
        if main_lv_nodes:
            main_pts = np.array([coord_mapping[n] for n in main_lv_nodes], dtype=float)
            main_tree = cKDTree(main_pts)
            for cnt, comp in comp_meter_cnt:
                if comp is main_comp or cnt == 0:
                    continue
                # find the node in this small component nearest to any main-comp LV node
                for n in comp:
                    if n not in coord_mapping:
                        continue
                    d, i = main_tree.query(np.array(coord_mapping[n], dtype=float))
                    bridge_target = main_lv_nodes[int(i)]
                    Rb = (float(d) / 1000.0) * conductorResistance
                    Xb = Rb * 0.1
                    G.add_edge(n, bridge_target, weight=float(d),
                               resistance=Rb, reactance=Xb, lc_bridge=True)
                    logging.info(
                        "calculateNetworkLoadCenter: bridged isolated meter comp "
                        "(meter_count=%d) node %s -> main-comp node %s  dist=%.2f m",
                        cnt, n, bridge_target, d)
                    break  # one bridge per isolated component is enough
    ...
    # track nodes actually reached by at least one meter; unreached nodes stay at 0
    # which looks like "best" to argmin — so we must exclude them explicitly
    reached = np.zeros(len(node_list), dtype=bool)
    ...
    if len(valid_meters) == 0:
        logging.warning(
            "No meters with non-zero loads; returning geometric centroid of meter locations "
            "(not [0,0]) to prevent TR placement at origin."
        )
        return np.mean(meterLocations, axis=0)
        ...
        for cluster_id in range(num_clusters):
            ...
            try:
                ...
                for node, dist in distances.items():
                    if node in node_id_to_index:
                        idx = node_id_to_index[node]
                        sum_distances[idx] += cluster_total_load * dist
                        reached[idx] = True
            except:
    ...
    # ใช้เฉพาะ node ที่ถูก reach จากมิเตอร์อย่างน้อย 1 ตัว
    # node ที่ไม่ถูก reach (disconnected component ว่าง) จะคงค่า 0 ไว้
    # ซึ่ง argmin จะเลือกผิดพลาด → บังคับเป็น inf แทน
    if np.any(reached):
        search_scores = np.where(reached, sum_distances, np.inf)
    else:
        search_scores = sum_distances  # fallback: ใช้ค่าเดิมถ้าไม่มี node ถูก reach เลย
    best_node_idx = np.argmin(search_scores)
...
def farthest_meter_from_initial_transformer_log(G_init, tNode_init, mNodes_init,
                                                meterLocations=None, peano=None, *, prefix="[INIT]"):
    dists = []
    for mNode in mNodes_init:
        try:
            d = nx.shortest_path_length(G_init, tNode_init, mNode, weight='weight')
        except nx.NetworkXNoPath:
            d = float("inf")
        dists.append(d)

    dists_arr = np.asarray(dists, dtype=float)
    finite = np.isfinite(dists_arr)

    if not np.any(finite):
        logging.warning(f"{prefix} No reachable meters from initial transformer.")
        return None, None, None, None

    finite_idx = np.where(finite)[0]
    i_far = int(finite_idx[np.argmax(dists_arr[finite])])

    max_dist = float(dists_arr[i_far])
    node_far = mNodes_init[i_far]
    xy_far = None if meterLocations is None else tuple(meterLocations[i_far])

    peano_id = None
    if peano is not None and 0 <= i_far < len(peano):
        peano_id = peano[i_far]

    logging.info(
        f"{prefix} Farthest meter: peano={peano_id}, node={node_far}, xy={xy_far}, "
        f"dist_along_network={max_dist:.3f}"
    )
    return max_dist, i_far, node_far, xy_far
...
def findSplittingPoint(G, transformerNode, meterNodes, coord_mapping, powerFactor, initialVoltage, candidate_index=0):
    """
    หา splitting edge โดยพยายามเลือก edge บนสายก่อน แล้วค่อย fallback ไป edge ที่ติด meter node
    แต่จะไม่เลือก edge ที่ทำให้กลุ่มใดกลุ่มหนึ่งมี meter น้อยเกินไป เช่น 1/45
    """
    global _EDGE_DF_CACHE
    logging.info("Finding splitting point by load balance difference on edges...")

    meter_set = set(meterNodes)
    total_meters_all = len(meterNodes)
    if total_meters_all < 2:
        logging.warning("Need at least 2 meters to split; found %d", total_meters_all)
        _EDGE_DF_CACHE = None
        return None, None, None, []

    node_loads = {n: (G.nodes[n].get('load_A', 0) + G.nodes[n].get('load_B', 0) + G.nodes[n].get('load_C', 0)) for n in G.nodes()}

    # ถ้า transformer ไปอยู่ component เล็กผิดปกติ ให้หา candidate จาก component ที่มี meter มากที่สุดแทน
    comps = list(nx.connected_components(G))
    comp_meter_counts = [len(meter_set.intersection(comp)) for comp in comps]
    source_node = transformerNode
    if comps:
        tx_comp_idx = next((i for i, comp in enumerate(comps) if transformerNode in comp), None)
        best_comp_idx = int(np.argmax(comp_meter_counts)) if comp_meter_counts else None
        tx_meters = comp_meter_counts[tx_comp_idx] if tx_comp_idx is not None else 0
        best_meters = comp_meter_counts[best_comp_idx] if best_comp_idx is not None else 0
        if best_comp_idx is not None and best_meters > max(tx_meters, 1):
            meters_in_best = [m for m in meterNodes if m in comps[best_comp_idx]]
            if meters_in_best:
                source_node = meters_in_best[0]
                logging.warning(
                    "Transformer component has only %d meters while largest component has %d meters; "
                    "using largest meter component as split-search source.", tx_meters, best_meters
                )

    T = nx.dfs_tree(G, source=source_node)
    ...
    total_load = computeCumulativeLoad(source_node)
    ...
    total_meters = cum_meter(source_node)
    if total_meters < 2:
        logging.warning("Split-search source reaches only %d meters; cannot create balanced split.", total_meters)
        _EDGE_DF_CACHE = None
        return None, None, None, []

    # กัน edge แบบ 1/45 แต่ยังยอมให้ network เล็กมากแบ่ง 1/1 หรือ 1/2 ได้
    min_meters = 1 if total_meters <= 3 else max(2, int(np.ceil(0.10 * total_meters)))

    def _build_candidates_from_tree(allow_meter_adjacent=False, mode_name="line_edge"):
        cand = []
        for (n1, n2) in T.edges():
            if (not allow_meter_adjacent) and ((n1 in meter_set) or (n2 in meter_set)):
                continue
            child_load = cumulative_loads.get(n2)
            if child_load is None:
                child_load = computeCumulativeLoad(n2)
            parent_load = total_load - child_load
            meters_child = meter_counts.get(n2)
            if meters_child is None:
                meters_child = cum_meter(n2)
            meters_parent = total_meters - meters_child
            if meters_child < min_meters or meters_parent < min_meters:
                continue
            cand.append({
                'Edge': (n1, n2),
                'Edge_Diff': abs(child_load - parent_load),
                'Load_G1': parent_load,
                'Load_G2': child_load,
                'Meters_G1': meters_parent,
                'Meters_G2': meters_child,
                'Candidate_Mode': mode_name,
                'Touches_Meter': (n1 in meter_set) or (n2 in meter_set),
            })
        return cand

    rows = _build_candidates_from_tree(allow_meter_adjacent=False, mode_name="line_edge")
    if not rows:
        logging.warning("No line-edge candidate found; fallback to meter-adjacent edges with min_meters=%d.", min_meters)
        rows = _build_candidates_from_tree(allow_meter_adjacent=True, mode_name="meter_node_fallback")

    rows.sort(key=lambda r: (r['Edge_Diff'], abs(r['Meters_G1'] - r['Meters_G2'])))

    # กรองให้เลือก bridge edges ก่อน (edges ที่ลบออกแล้ว disconnect graph จริง)
    # ป้องกันการเลือก edge ที่อยู่ใน ring/loop ซึ่งลบออกแล้วยังเชื่อมกันอยู่
    try:
        bridge_set = set(nx.bridges(G))
        bridge_rows = [r for r in rows
                       if r['Edge'] in bridge_set or (r['Edge'][1], r['Edge'][0]) in bridge_set]
        if bridge_rows:
            logging.info("Bridge-edge filter: %d bridge candidates (out of %d total).", len(bridge_rows), len(rows))
            rows = bridge_rows
        else:
            logging.warning(
                "No bridge-edge candidates found (network may be fully 2-connected). "
                "Using all %d candidates — partition will use DFS tree fallback.", len(rows)
            )
    except Exception as _bridge_err:
        logging.warning("Bridge detection failed (%s); using all candidates.", _bridge_err)
    ...
    if not rows:
        _EDGE_DF_CACHE = None
        pd.DataFrame(columns=['splitting_index','Edge','Edge_Diff','N1_X','N1_Y','N2_X','N2_Y','Load_G1','Load_G2','Meters_G1','Meters_G2','Candidate_Mode','Touches_Meter']).to_csv(csv_path, index=False)
        logging.warning("No valid splitting edges after balanced fallback. total_meters=%d, reachable_meters=%d, min_meters=%d", total_meters_all, total_meters, min_meters)
        return None, None, None, []
    ...
    for r in rows:
        ...
        rr = dict(r)
        rr.update({'N1_X': x1, 'N1_Y': y1, 'N2_X': x2, 'N2_Y': y2})
        out_rows.append(rr)

    edge_diffs_df = pd.DataFrame(out_rows)
    ...
    logging.info(f"Splitting edges info saved to CSV: {csv_path}. Found {len(rows)} edges.")

    if candidate_index < 0 or candidate_index >= len(rows):
        logging.error("Candidate index out of range, using index 0 instead.")
        candidate_index = 0

    best = rows[candidate_index]
    best_edge = best['Edge']
    best_edge_diff = best['Edge_Diff']
    u, v = best_edge
    splitting_point_coord = (np.array(coord_mapping[u]) + np.array(coord_mapping[v])) / 2

    logging.info("Splitting edge chosen (candidate=%d): %s, diff=%.2f, meters=(%d,%d), mode=%s", candidate_index, best_edge, best_edge_diff, best['Meters_G1'], best['Meters_G2'], best['Candidate_Mode'])
    ...
    w.field("Meters_G1", "N")
    w.field("Meters_G2", "N")
    w.field("Mode", "C", size=32)
    for i, row in edge_diffs_df.iterrows():
        if np.isnan(row['N1_X']) or np.isnan(row['N2_X']):
            continue
        w.line([[[row['N1_X'], row['N1_Y']], [row['N2_X'], row['N2_Y']]]])
        w.record(int(i), int(row['splitting_index']), float(row['Edge_Diff']), int(row['Meters_G1']), int(row['Meters_G2']), str(row['Candidate_Mode'])[:32])
    w.close()
    logging.info(f"Shapefile of edges exported: {shp_path}")
    return best_edge, splitting_point_coord, best_edge_diff, [(r['Edge'], r['Edge_Diff']) for r in rows]


def partitionNetworkAtPoint(G, transformerNode, meterNodes, splitting_edge=None, coord_mapping=None):
    """
    แบ่งกลุ่มจากสองฝั่งของ splitting_edge โดยตรง
    ไม่ใช้ transformerNode เป็นตัวตัดสินกลุ่ม

    Group 1 = ฝั่ง u ของ splitting_edge
    Group 2 = ฝั่ง v ของ splitting_edge
    """
    logging.info("Partitioning network by removing splitting edge only...")

    if splitting_edge is None:
        logging.warning("No splitting_edge provided.")
        return list(meterNodes), []

    u, v = splitting_edge

    G_cut = G.copy()

    if not G_cut.has_edge(u, v):
        logging.warning(f"Splitting edge {splitting_edge} not found in graph.")
        return list(meterNodes), []

    G_cut.remove_edge(u, v)
    logging.info(f"Removed splitting edge: {splitting_edge}")

    if u not in G_cut or v not in G_cut:
        logging.warning("Splitting edge nodes not found after cut.")
        return list(meterNodes), []

    comp_u = nx.node_connected_component(G_cut, u)
    comp_v = nx.node_connected_component(G_cut, v)

    if comp_u == comp_v:
        # splitting_edge ไม่ใช่ bridge — ลบแล้ว graph ยังเชื่อมกันอยู่ (มี ring/cycle)
        # ใช้ DFS tree subtree แทน component-based partition
        logging.warning(
            "Splitting edge %s is NOT a bridge (graph still connected after removal). "
            "Falling back to DFS tree partition.", splitting_edge
        )
        source = transformerNode if (transformerNode in G_cut) else u
        T_dfs = nx.dfs_tree(G_cut, source=source)
        subtree_v = set(nx.dfs_preorder_nodes(T_dfs, source=v))
        group1_meters = [m for m in meterNodes if m not in subtree_v]
        group2_meters = [m for m in meterNodes if m in subtree_v]
    else:
        group1_meters = [m for m in meterNodes if m in comp_u]
        group2_meters = [m for m in meterNodes if m in comp_v]

    # เช็ก meter ที่ไม่อยู่ทั้งสองฝั่ง เผื่อ graph มี component อื่น
    assigned = set(group1_meters) | set(group2_meters)
    unassigned = [m for m in meterNodes if m not in assigned]

    if unassigned:
        logging.warning(
            "Found %d meters not assigned to either side of splitting edge.",
            len(unassigned)
        )

        # fallback: จัดเข้าใกล้ฝั่ง u/v ตาม shortest path (graph distance)
        # ถ้า meter isolated จากทั้ง u และ v → ใช้ Euclidean distance จาก coord_mapping
        cm = coord_mapping or {}
        for m in unassigned:
            try:
                du = nx.shortest_path_length(G_cut, u, m, weight="weight")
            except Exception:
                du = float("inf")

            try:
                dv = nx.shortest_path_length(G_cut, v, m, weight="weight")
            except Exception:
                dv = float("inf")

            if du == float("inf") and dv == float("inf") and m in cm:
                # meter ถูกตัดขาดจาก network โดยสิ้นเชิง → Euclidean fallback
                mx, my = cm[m]
                eu = np.hypot(mx - cm[u][0], my - cm[u][1]) if u in cm else float("inf")
                ev = np.hypot(mx - cm[v][0], my - cm[v][1]) if v in cm else float("inf")
                if eu <= ev:
    ...
    logging.info(
        "Partition by split edge done: Group1(side u)=%d meters, Group2(side v)=%d meters",
        len(group1_meters),
        len(group2_meters)
    )
...
def get_allowed_candidate_coords_after_split(G, coord_mapping, splitting_edge,
                                             group_meter_nodes, all_meter_nodes):
    """
    สร้างขอบเขต candidate สำหรับวางหม้อแปลงจาก graph หลังตัด splitting_edge

    หลักการ:
    1) copy G
    2) remove splitting_edge
    3) หา component ที่มี meter ของกลุ่มนั้นมากที่สุด
    4) คืนพิกัด node ใน component นั้น ยกเว้น meter node
    """
    if not group_meter_nodes:
        return []

    G_cut = G.copy()

    if splitting_edge is not None and G_cut.has_edge(*splitting_edge):
        G_cut.remove_edge(*splitting_edge)
    else:
        logging.warning("get_allowed_candidate_coords_after_split: splitting_edge not found in graph")

    group_meter_set = set(group_meter_nodes)
    all_meter_set = set(all_meter_nodes)

    comps = list(nx.connected_components(G_cut))
    if not comps:
        return []

    # ตรวจว่า split edge เป็น bridge หรือไม่ (ถ้าไม่ใช่ comp_u == comp_v)
    u, v = splitting_edge if splitting_edge is not None else (None, None)
    non_bridge_fallback = False
    if u is not None and v is not None and u in G_cut and v in G_cut:
        if nx.node_connected_component(G_cut, u) == nx.node_connected_component(G_cut, v):
            non_bridge_fallback = True

    if non_bridge_fallback:
        # splitting_edge ไม่ใช่ bridge — ใช้ DFS tree subtree เพื่อกำหนดพื้นที่ candidate
        logging.warning(
            "get_allowed_candidate_coords_after_split: non-bridge edge %s; using DFS tree subtree.", splitting_edge
        )
        T_dfs = nx.dfs_tree(G, source=u)
        subtree_v = set(nx.dfs_preorder_nodes(T_dfs, source=v))

        # กำหนดว่า group นี้อยู่ฝั่งไหนของ DFS tree
        meters_in_subtree_v = group_meter_set & subtree_v
        meters_not_in_subtree = group_meter_set - subtree_v
        if len(meters_in_subtree_v) >= len(meters_not_in_subtree):
            # group อยู่ฝั่ง v (subtree of v)
            candidate_nodes = subtree_v - all_meter_set
        else:
            # group อยู่ฝั่ง u (complement of subtree_v)
            candidate_nodes = (set(G.nodes()) - subtree_v) - all_meter_set
        allowed_coords = [tuple(coord_mapping[n]) for n in candidate_nodes if n in coord_mapping]
    else:
        best_comp = max(
            comps,
            key=lambda c: len(group_meter_set.intersection(c))
        )
        allowed_coords = []
        for n in best_comp:
            if n in coord_mapping and n not in all_meter_set:
                allowed_coords.append(tuple(coord_mapping[n]))

    logging.info(
        "Allowed split-side candidate coords: group_meters=%d, allowed_nodes=%d",
        len(group_meter_nodes),
        len(allowed_coords)
    )
...
def performForwardBackwardSweepAndDivideLoads(G, transformerNode, meterNodes, coord_mapping, powerFactor, initialVoltage, splitting_edge):
    ...
    group1_meters, group2_meters = partitionNetworkAtPoint(G, transformerNode, meterNodes, splitting_edge, coord_mapping=coord_mapping)
...
def optimizeTransformerLocationOnLVCond3(meterLocations, phase_loads, initialTransformerLocation,
    ...
    # Pre-compute coord_snap_map and line_length_map once — reused across all optimizer iterations
    _all_coords = []
    for _l in lvLines:
        _all_coords.extend([(x, y) for x, y in zip(_l['X'], _l['Y']) if not (np.isnan(x) or np.isnan(y))])
    _cached_snap_map = snap_coordinates_to_tolerance(list(set(_all_coords)), SNAP_TOLERANCE)
    _cached_len_map = None
    if lvData is not None:
        try:
            _cached_len_map = build_line_length_map_from_json(
                json_input=lvData, coord_snap_map=_cached_snap_map,
                length_field="SHAPE.LEN", fallback_fields=("Shape_Leng", "SHAPE_Leng"), unit_factor=1.0
            )
        except Exception as _e:
            logging.warning(f"[cache] line_length_map build failed, will use Euclidean: {_e}")
    ...
    def obj_func(x):
        return objectiveFunction(x, meterLocations, phase_loads, initialVoltage,
                                 ...
                                 _coord_snap_map=_cached_snap_map, _line_length_map=_cached_len_map)
...
# สัดส่วน Load/Transformer และการกระจายโหลดในกิ่ง
def calculate_branch_load_balance_penalty(G, tNode, meter_node_to_load, total_group_load):
    """
    ตรวจว่าโหลดกระจุกอยู่กิ่งเดียวจากตำแหน่ง TR หรือไม่

    วิธีคิด:
    - ดู neighbor รอบ tNode
    - แต่ละ neighbor คือทิศทาง/กิ่งที่โหลดไหลออกจาก TR
    - รวมโหลด meter ที่อยู่ในแต่ละกิ่ง
    - ถ้าโหลดกิ่งที่มากที่สุดกินสัดส่วนสูงเกินไป ให้ penalty สูง

    Returns:
        branch_penalty: 0.0 = ดี, 1.0 = แย่มาก
        max_branch_frac: สัดส่วนโหลดสูงสุดในกิ่งเดียว
        branch_count: จำนวนกิ่งที่มีโหลด
    """
    if total_group_load <= 0 or tNode not in G:
        return 1.0, 1.0, 0

    neighbors = list(G.neighbors(tNode))
    if not neighbors:
        return 1.0, 1.0, 0
    ...
    for nb in neighbors:
        # ตัด edge ระหว่าง tNode กับ neighbor ชั่วคราว
        G_tmp = G.copy()
        if G_tmp.has_edge(tNode, nb):
            G_tmp.remove_edge(tNode, nb)

        # component ที่อยู่ฝั่ง neighbor คือโหลดในกิ่งนั้น
        if nb not in G_tmp:
            continue

        branch_nodes = nx.node_connected_component(G_tmp, nb)
        load_sum = 0.0

        for m_node, load in meter_node_to_load.items():
            if m_node in branch_nodes:
                load_sum += float(load)

        if load_sum > 0:
            branch_loads.append(load_sum)

    if not branch_loads:
        return 1.0, 1.0, 0

    max_branch_load = max(branch_loads)
    max_branch_frac = max_branch_load / total_group_load
    branch_count = len(branch_loads)

    # ถ้ามีโหลดอยู่แค่กิ่งเดียว ถือว่าอยู่ปลายกิ่ง/โหลดทางเดียว
    if branch_count <= 1:
        return 1.0, max_branch_frac, branch_count

    # threshold:
    # <= 0.70 ดีมาก penalty = 0
    # >= 0.90 แย่มาก penalty = 1
    # ระหว่างนั้น scale linearly
    branch_penalty = (max_branch_frac - 0.70) / (0.90 - 0.70)
    branch_penalty = float(np.clip(branch_penalty, 0.0, 1.0))

    return branch_penalty, max_branch_frac, branch_count
...
def objectiveFunction(transformerLocation, meterLocations, phase_loads, initialVoltage,
                     ...
                     conductorReactance=None, lvData=None, svcLines=None,
                     _coord_snap_map=None, _line_length_map=None):
    ...
    G, tNode, mNodes, nm, cm = buildLVNetworkWithLoads(
        ...
        conductorReactance=conductorReactance,
        use_shape_length=(lvData is not None) or (_line_length_map is not None),
        lvData=lvData,
        svcLines=svcLines,
        snap_tolerance=SNAP_TOLERANCE,
        coord_snap_map=_coord_snap_map,
        line_length_map=_line_length_map,
    )
    ...
    # --- load-weighted geometric center เป็น reference (ใช้แทน network 1-median) ---
    # network 1-median จะอยู่ที่ meter ใดก็ตามที่มีโหลด >50% ของกลุ่ม ทำให้ TR ถูกดึง
    # ไปปลายกิ่งถ้ามี outlier meter โหลดสูง geometric center กระจายน้ำหนักได้สม่ำเสมอกว่า
    _wt = phase_loads['A'] + phase_loads['B'] + phase_loads['C']
    _wt = np.maximum(_wt, 1e-9)
    netCenter = np.average(meterLocations, axis=0, weights=_wt)
    ...
    # --- penalty กัน TR ไปอยู่ปลายกิ่ง / โหลดกระจุกอยู่ทางเดียว ---
    branch_penalty, max_branch_frac, branch_count = calculate_branch_load_balance_penalty(
        G,
        tNode,
        meter_node_to_load,
        total_group_load
    ...
    if load_center_only:
        ...
        load_center_weight  = 80.0
        local_load_weight   = 80.0
        branch_balance_weight = 0
    else:
    ...
    score = (
        ...
        branch_balance_weight * branch_penalty
    )

    logging.info(
        "Objective at %s -> Vdrop=%.2f, Loss=%.2f, dCenter=%.2f, "
        "local_frac=%.3f, max_branch_frac=%.3f, branch_count=%d, branch_penalty=%.3f, score=%.2f",
        transformerLocation,
        totalVoltageDrop,
        totalPowerLoss,
        distanceFromNetworkCenter,
        local_frac,
        max_branch_frac,
        branch_count,
        branch_penalty,
...
def _collect_lv_vertices_by_phase_priority(lvData, phase_priority=(7, 6, 5, 4, 3, 2, 1)):
    """
    เลือกสาย LV ตามลำดับ PHASEDESIGNATION/PHASEDESIG แล้วเก็บ 'ทุก vertex' ของเส้น
    คืนค่า:
      pts_set, selected_phase_code
    """
    if lvData is None:
        return set(), None
    ...
    try:
        ...
        logging.warning(f"Cannot read lvData: {e}")
        return set(), None

    idx_phase = None
    for fn in ("PHASEDESIGNATION", "PHASEDESIG"):
        if fn in fields:
            idx_phase = fields.index(fn)
            break

    if idx_phase is None:
        logging.warning("lvData has no PHASEDESIGNATION/PHASEDESIG field")
        return set(), None

    for ph_target in phase_priority:
        pts = set()
        for rec, shp in zip(recs, shps):
            try:
                ph = int(float(rec[idx_phase]))
            except Exception:
                continue
            if ph != ph_target:
                continue
        ...
        if pts:
            logging.info("Selected PHASEDESIGNATION=%s with %d LV vertices", ph_target, len(pts))
            return pts, ph_target
...
def optimizeGroup(meterLocations, phase_loads, initialTransformerLocation,
                  ...
                  lvData=None, svcLines=None, use_phase_filter=False,allowed_candidate_coords=None):
    """
    ...
    - กรองโหนดตาม PHASEDESIGNATION priority: 7 -> 6 -> 5 -> 4 -> 3 -> 2 -> 1
    ...
    buffer = max(5.0, min(25.0, 0.05 * diag))
    min_x -= buffer; max_x += buffer; min_y -= buffer; max_y += buffer

    # 2) สร้างกราฟชั่วคราวเฉพาะกลุ่ม
    use_shape_length = (lvData is not None and not callable(lvData))
    lvData_for_length = lvData if use_shape_length else None
    ...
    conductorReactance=conductorReactance,
    use_shape_length=use_shape_length,
    lvData=lvData_for_length,
    svcLines=svcLines,
    snap_tolerance=SNAP_TOLERANCE
    )

    # mapping มิเตอร์ -> โหลด
    meter_node_to_load = {mNodes_temp[i]: float(total_loads_arr[i]) for i in range(len(mNodes_temp))}

    # 3) หา component หลักจากมิเตอร์
    comp_id_of = {}
    ...
    logging.info("Candidates before phase filter: %d", len(candidates))
    # บังคับ candidate ให้เป็น node ที่อยู่บน LV conductor จริงเท่านั้น
    # กัน service-side node / meter-side service node ที่พิกัดทับมิเตอร์หลุดมาเป็นตำแหน่ง TR
    # ---------- Lock candidate ให้อยู่ฝั่งเดียวกับกลุ่มหลังตัด splitting_edge ----------
    if allowed_candidate_coords:
        allowed_arr = np.asarray(allowed_candidate_coords, dtype=float)

        if len(allowed_arr) > 0 and len(candidates) > 0:
            allowed_tree = cKDTree(allowed_arr)
            tol_allowed = max(1.0, float(SNAP_TOLERANCE))

            candidates_in_split_side = []

            for n in candidates:
                xy = np.asarray(coord_mapping_temp[n], dtype=float)
                d_allowed, _ = allowed_tree.query(xy, k=1)

                if float(d_allowed) <= tol_allowed:
                    candidates_in_split_side.append(n)

            logging.info(
                "Split-side candidate filter: %d -> %d nodes",
                len(candidates),
                len(candidates_in_split_side)
            )

            if candidates_in_split_side:
                candidates = candidates_in_split_side
            else:
                logging.warning(
                    "No candidates matched split-side allowed coords. "
                    "Returning nearest allowed coordinate to initial load center."
                )
                guess = np.asarray(initialTransformerLocation, dtype=float)
                _, idx_allowed = allowed_tree.query(guess, k=1)
                return allowed_arr[int(idx_allowed)]

    lv_coord_set = set()
    for line in lvLines:
        for x, y in zip(line['X'], line['Y']):
            if not (np.isnan(x) or np.isnan(y)):
                lv_coord_set.add((float(x), float(y)))

    if lv_coord_set and candidates:
        lv_pts = np.asarray(list(lv_coord_set), dtype=float)
        lv_tree = cKDTree(lv_pts)

        tol_lv_candidate = max(1.0, float(SNAP_TOLERANCE))

        candidates_lv_only = []
        for n in candidates:
            xy = np.asarray(coord_mapping_temp[n], dtype=float)
            d_lv, _ = lv_tree.query(xy, k=1)

            if float(d_lv) <= tol_lv_candidate:
                candidates_lv_only.append(n)

        if candidates_lv_only:
            logging.info(
                "LV-only candidate filter: %d -> %d nodes",
                len(candidates),
                len(candidates_lv_only)
            )
            candidates = candidates_lv_only
        else:
            logging.warning(
                "LV-only candidate filter removed all candidates; keep original candidates."
        )
    selected_phase_code = "DISABLED"

    if use_phase_filter:
        # ---- กรอง PHASEDESIGNATION แบบ priority: 7 -> 6 -> 5 -> 4 -> 3 -> 2 -> 1 ----
        selected_phase_code = None
        phase_priority = (7, 6, 5, 4, 3, 2, 1)

        if callable(lvData):
            logging.warning(
                "lvData is callable; cannot collect PHASEDESIGNATION vertices. "
                "Using component candidates."
            )
        else:
            lv_phase_pts, selected_phase_code = _collect_lv_vertices_by_phase_priority(
                lvData,
                phase_priority=phase_priority
            )

            if lv_phase_pts:
                lv_phase_arr = np.asarray(list(lv_phase_pts), dtype=float)
                tree_phase = cKDTree(lv_phase_arr)

                tol_match = max(1.0, float(SNAP_TOLERANCE))
                cand_before = len(candidates)
                candidates_phase = []

                for n in candidates:
                    pt = np.asarray(coord_mapping_temp[n], dtype=float)
                    d, _ = tree_phase.query(pt, k=1)
                    if float(d) <= tol_match:
                        candidates_phase.append(n)

                logging.info(
                    "Phase-priority candidate filter selected PHASEDESIGNATION=%s : %d -> %d nodes",
                    selected_phase_code,
                    cand_before,
                    len(candidates_phase)
                )

                if candidates_phase:
                    candidates = candidates_phase
                else:
                    logging.warning(
                        "Selected phase code=%s but no graph nodes matched LV vertices. "
                        "Using unfiltered candidates.",
                        selected_phase_code
                    )
            else:
                logging.warning(
                    "No LV lines found for PHASEDESIGNATION priority %s; using all candidates.",
                    phase_priority
                )
    else:
        logging.info(
            "PHASEDESIGNATION candidate filter is DISABLED. "
            "Using all component candidates."
        )
     # 5) Bounding-box clamp (ต้องมีเสมอ ก่อน radial/hull)
    cand_in_bounds = [
        n for n in candidates
        if (min_x <= coord_mapping_temp[n][0] <= max_x) and (min_y <= coord_mapping_temp[n][1] <= max_y)
    ]
    if not cand_in_bounds:
        logging.warning("BBox clamp removed all candidates; revert to phase-filtered candidates.")
        cand_in_bounds = candidates[:]
    # 6) Radial clamp รอบ centroid ของกลุ่ม
    cx, cy = meterLocations.mean(axis=0)
    dists  = np.hypot(meterLocations[:, 0] - cx, meterLocations[:, 1] - cy)
    r_max  = dists.max() if len(dists) > 0 else 0.0
    radial_margin = max(3.0, min(15.0, 0.03 * diag))
    radius = r_max + radial_margin

    _cand_backup = cand_in_bounds[:]   # ✅ บรรทัดเดียวพอ
    cand_in_bounds = [
        n for n in cand_in_bounds
        if np.hypot(coord_mapping_temp[n][0] - cx, coord_mapping_temp[n][1] - cy) <= radius
    ]
    if not cand_in_bounds:
        logging.warning("Radial clamp removed all candidates; revert to bbox candidates.")
    ...
    try:
        ...
        if len(meterLocations) >= 3:
            ...
            poly = meterLocations[hull.vertices]
            pcx, pcy = poly.mean(axis=0)
            scale = 1.08
            ...
            cand_in_bounds = [n for n in cand_in_bounds if path.contains_point(coord_mapping_temp[n])]
            if not cand_in_bounds:
    ...
    # 8) ถ้ายังว่าง ให้กลับไปใช้ candidates หลัง phase-filter
    if not cand_in_bounds:
        logging.warning("No candidates after clamps; falling back to phase-filtered candidates.")
        cand_in_bounds = candidates[:]

    # 9) local load fraction rule
    local_radius = max(30.0, 0.35 * diag)
    min_local_frac = 0.35

    best_coord = None
    best_score = float('inf')

    # Branch-balance pre-filter: reject candidates where >85% of load is on one branch.
    # Terminal nodes (branch_count=1) are the worst candidates (100% load on one side)
    # and are explicitly excluded — only _bb_f <= threshold passes.
    _MAX_BRANCH_FRAC = 0.85
    _pass_bb = []
    _bb_scores = []  # (max_branch_frac, node) for fallback ranking
    for _n in cand_in_bounds:
        if _n not in coord_mapping_temp:
            continue
        _, _bb_f, _bb_cnt = calculate_branch_load_balance_penalty(
            G_temp, _n, meter_node_to_load, total_group_load
        )
        _bb_scores.append((_bb_f, _n))
        if _bb_f <= _MAX_BRANCH_FRAC:
            _pass_bb.append(_n)
    if _pass_bb:
        logging.info(
            "Branch balance pre-filter: %d/%d candidates pass (max_frac<=%.2f)",
            len(_pass_bb), len(cand_in_bounds), _MAX_BRANCH_FRAC
        )
        cand_to_score = _pass_bb
    else:
        # All candidates exceed the balance threshold (highly radial network).
        # Fall back to the best quarter by max_branch_frac rather than terminal nodes.
        _bb_scores.sort(key=lambda x: x[0])
        _fallback_n = max(1, len(_bb_scores) // 4)
        cand_to_score = [_n for _, _n in _bb_scores[:_fallback_n]]
        logging.warning(
            "Branch balance pre-filter: all %d candidates exceed max_frac=%.2f; "
            "using best %d by branch fraction as fallback.",
            len(cand_in_bounds), _MAX_BRANCH_FRAC, len(cand_to_score)
        )

    for node_id in cand_to_score:
    ...
    if best_coord is None:
        logging.warning(
            ...
            "falling back to snapping init guess to nearest candidate node."
        )
        if candidates:
            ...
            best_coord = pts[int(idx)]
        else:
            logging.error("Component has no candidate nodes; returning initial transformer location.")
            return np.array(initialTransformerLocation, dtype=float)

    logging.info(
        "Discrete node-based optimization (component-locked, phase-priority=%s, local-load-aware) => %s, score=%.2f",
        selected_phase_code, best_coord, best_score
    )
    return best_coord
...
def rerun_process(candidate_index,
    ...
    group1_nodes, group2_nodes = partitionNetworkAtPoint(G, transformerNode, meterNodes, best_edge, coord_mapping=coord_mapping)
    if globals().get('tk_progress'): tk_progress.step()

    # 3) run sweep แยกโหลด (เหมือน main)
    voltages, branch_curr, _g1_fbs, _g2_fbs = performForwardBackwardSweepAndDivideLoads(
...
def refine_group_by_nested_split(group_name, meter_locs, phase_loads,
    ...
    A_nodes, B_nodes = partitionNetworkAtPoint(Gg, tNode_g, mNodes_g, best_edge, coord_mapping=cm_g)
...
def recursive_split_group(group_tag, meter_locs, phase_loads,
                          lv_lines, mv_lines, init_tx_location,
                          initial_voltage, conductor_resistance, power_factor,
                          conductor_reactance=None, lv_data=None, svc_lines=None,
                          max_group_kVA=RECURSIVE_SPLIT_MAX_KVA,
                          max_depth=RECURSIVE_SPLIT_MAX_DEPTH,
                          candidate_index=0, _depth=0, meter_idx=None):
    """
    แบ่งกลุ่มซ้ำ (recursive) จนกว่าโหลดกลุ่มจะ <= max_group_kVA
    ใช้ min_meters แบบเดียวกับ findSplittingPoint (10% ของมิเตอร์ในกลุ่ม, ขั้นต่ำ 2 หรือ 1
    ถ้ากลุ่มมี <=3 มิเตอร์) เป็นตัวตัดสินว่าจะแบ่งต่อได้ไหม — ไม่ตั้ง threshold จำนวนมิเตอร์แยกต่างหาก
    เพราะ findSplittingPoint เองจะคืน None เมื่อแบ่งแล้วฝั่งใดฝั่งหนึ่งมีมิเตอร์น้อยเกินไปอยู่แล้ว
    คืนค่าเป็น list ของกลุ่มใบ (leaf) แต่ละอันมี tag/meter_locs/phase_loads/tx_loc/load_kVA/n_meters/meter_idx
    meter_idx (ถ้าให้มา) คือ index ตัวเดิมใน array ต้นทาง (global) ใช้ตรวจสอบ/export ว่ามิเตอร์ตัวไหนอยู่ leaf ไหนจริงๆ
    (ไม่ใช่ค่าประมาณจากระยะทาง) — ตามการตัด edge จริงใน partitionNetworkAtPoint
    """
    total_kW  = sum(phase_loads[p].sum() for p in 'ABC')
    total_kVA = total_kW / power_factor
    n_meters  = len(meter_locs)
    if meter_idx is None:
        meter_idx = np.arange(n_meters)

    def _leaf():
        return [{'tag': group_tag, 'meter_locs': meter_locs, 'phase_loads': phase_loads,
                 'tx_loc': init_tx_location, 'load_kVA': total_kVA, 'n_meters': n_meters,
                 'meter_idx': meter_idx}]

    if total_kVA <= max_group_kVA:
        logging.info(f"[{group_tag}] leaf: load={total_kVA:.1f} kVA <= max_group_kVA={max_group_kVA}")
        return _leaf()
    if _depth >= max_depth:
        logging.warning(f"[{group_tag}] leaf: reached max_depth={max_depth} (load={total_kVA:.1f} kVA)")
        return _leaf()

    split = refine_group_by_nested_split(
        group_tag, meter_locs, phase_loads, lv_lines, mv_lines,
        init_tx_location, initial_voltage, conductor_resistance, power_factor,
        candidate_index=candidate_index, conductor_reactance=conductor_reactance,
        lv_data=lv_data, svc_lines=svc_lines)

    if split is None or split['subA']['tx'] is None or split['subB']['tx'] is None:
        # findSplittingPoint หา bridge edge ที่ผ่าน min_meters ไม่ได้ (เช่น mesh แน่น หรือมิเตอร์เหลือน้อยเกินจะแบ่ง)
        logging.warning(f"[{group_tag}] leaf: cannot split further (load={total_kVA:.1f} kVA, meters={n_meters})")
        return _leaf()

    leaves = []
    for suffix in ('A', 'B'):
        idx = split[f'sub{suffix}']['idx']
        if idx.size == 0:
            continue
        leaves.extend(recursive_split_group(
            f"{group_tag}-{suffix}", meter_locs[idx],
            {ph: phase_loads[ph][idx] for ph in 'ABC'},
            lv_lines, mv_lines, split[f'sub{suffix}']['tx'],
            initial_voltage, conductor_resistance, power_factor,
            conductor_reactance=conductor_reactance, lv_data=lv_data, svc_lines=svc_lines,
            max_group_kVA=max_group_kVA, max_depth=max_depth,
            candidate_index=candidate_index, _depth=_depth + 1, meter_idx=meter_idx[idx]))
    return leaves


def run_recursive_optimisation(last_result, lv_lines, mv_lines,
                               initial_voltage, conductor_resistance, power_factor,
                               conductor_reactance=None, lv_data=None, svc_lines=None,
                               max_group_kVA=RECURSIVE_SPLIT_MAX_KVA,
                               max_depth=RECURSIVE_SPLIT_MAX_DEPTH,
                               peano=None):
    """
    เวอร์ชัน headless-safe ของ run_nested_optimisation — แบ่งกลุ่มซ้ำจนโหลดแต่ละใบ <= max_group_kVA
    แทนที่จะ fix ไว้แค่ระดับเดียว (subA/subB) ไม่พึ่ง messagebox จึงเรียกได้ทั้ง GUI และ headless
    ถ้าส่ง peano (global array เดียวกับที่ main() ใช้) มาด้วย จะ export nested_meter_assignment.csv
    ที่บอกว่ามิเตอร์แต่ละตัว (ตาม Peano จริง) อยู่ leaf ไหน — มาจาก idx การตัด edge จริง ไม่ใช่ระยะทางประมาณ
    """
    if last_result is None:
        logging.warning("run_recursive_optimisation: ไม่มี latest_split_result – ต้อง Run Process ก่อน")
        return []

    leaves = []
    for tag, key in (("Group1", 'group1'), ("Group2", 'group2')):
        g = last_result.get(key)
        if g is None or g['idx'].size == 0:
            continue
        leaves.extend(recursive_split_group(
            tag, g['meter_locs'], g['phase_loads'], lv_lines, mv_lines, g['tx_loc'],
            initial_voltage, conductor_resistance, power_factor,
            conductor_reactance=conductor_reactance, lv_data=lv_data, svc_lines=svc_lines,
            max_group_kVA=max_group_kVA, max_depth=max_depth, meter_idx=g['idx']))

    result_lines, loss_lines = [], []
    extra_pts, extra_attrs = [], []
    for leaf in leaves:
        idx_local = np.arange(leaf['n_meters'])
        res, loss = summarise_subgroup(
            leaf['tag'], idx_local, leaf['tx_loc'],
            leaf['meter_locs'], leaf['phase_loads'], lv_lines, mv_lines,
            initial_voltage, conductor_resistance, power_factor,
            conductor_reactance=conductor_reactance, lv_data=lv_data, svc_lines=svc_lines
        )
        if res:
            result_lines.append(res)
            loss_lines.append(loss)
        if leaf['tx_loc'] is not None:
            extra_pts.append(tuple(leaf['tx_loc']))
            extra_attrs.append({"Name": f"{leaf['tag']} TX"})

    if result_lines:
        logging.info("############ Result_Subgroup (recursive) ############")
        for line in result_lines:
            logging.info(line)
    if loss_lines:
        logging.info("############ Loss Report_Subgroup (recursive) ############")
        for line in loss_lines:
            logging.info(line)
    ...
    if extra_pts:
        ...
        logging.info(f"Recursive optimisation finished – {len(leaves)} leaf groups exported.")

    if peano is not None:
        rows = []
        for leaf in leaves:
            leaf_peano = peano[leaf['meter_idx']]
            for p, (mx, my) in zip(leaf_peano, leaf['meter_locs']):
                rows.append({'Peano': p, 'leaf_tag': leaf['tag'], 'Meter X': mx, 'Meter Y': my})
        if rows:
            folder_path = OUT_DIR
            ensure_folder_exists(folder_path)
            pd.DataFrame(rows).to_csv(f"{folder_path}/nested_meter_assignment.csv", index=False)
            logging.info(f"Nested meter assignment (actual, from graph partition) saved: "
                        f"{folder_path}/nested_meter_assignment.csv ({len(rows)} meters).")
...
def runRecursiveSplit(max_group_kVA=None, max_split_depth=None, prompt_for_params=True):
    """
    ปุ่ม "Split Subgroups (Recursive)" — แบ่ง Group1/Group2 ที่ได้จาก Run Process ต่อไปเรื่อยๆ
    จนกว่าโหลดแต่ละกลุ่มใบจะ <= max_group_kVA (ดูรายละเอียดที่ run_recursive_optimisation)

    เรียกได้ 2 แบบ:
      - runRecursiveSplit()                              -> ถาม kVA/ความลึกผ่าน dialog (ใช้ตอนกดปุ่มใน GUI)
      - runRecursiveSplit(50.0, 2, prompt_for_params=False) -> ไม่ถาม ใช้ค่าที่ส่งมาตรงๆ (เรียกจาก UI อื่น/สคริปต์)

    ผลลัพธ์ (โหลด/loss ต่อกลุ่ม, shapefile ตำแหน่ง TX, nested_meter_assignment.csv) จะถูก log/export
    เหมือนตอนรันแบบ headless ด้วย --recursive-split ทุกประการ — ฟังก์ชันนี้เป็นแค่ wrapper บาง ๆ
    รอบ run_recursive_optimisation() เพื่อผูกกับปุ่ม/ถามค่าพารามิเตอร์เท่านั้น
    """
    global latest_split_result, lvLines, mvLines, initialVoltage, conductorResistance, powerFactor, conductorReactance, lvData, svcLines, peano
    global RECURSIVE_SPLIT_MAX_KVA, RECURSIVE_SPLIT_MAX_DEPTH
    ...
    if max_group_kVA is None:
        max_group_kVA = RECURSIVE_SPLIT_MAX_KVA
    if max_split_depth is None:
        max_split_depth = RECURSIVE_SPLIT_MAX_DEPTH

    if prompt_for_params:
        kva_in = simpledialog.askfloat(
            "Recursive Split",
            "เพดานโหลดต่อกลุ่มใบ (kVA) — หยุดแบ่งเมื่อกลุ่มมีโหลด <= ค่านี้:",
            initialvalue=max_group_kVA, minvalue=0.0)
        if kva_in is None:
            return []  # ผู้ใช้กด Cancel
        max_group_kVA = kva_in

        depth_in = simpledialog.askinteger(
            "Recursive Split",
            "ความลึกสูงสุดของการแบ่ง (safety valve, เช่น 1 = แบ่งแค่ชั้นเดียว = 4 กลุ่ม):",
            initialvalue=max_split_depth, minvalue=1, maxvalue=10)
        if depth_in is None:
            return []
        max_split_depth = depth_in

    leaves = run_recursive_optimisation(
        latest_split_result, lvLines, mvLines,
        initialVoltage, conductorResistance, powerFactor,
        conductor_reactance=conductorReactance, lv_data=lvData, svc_lines=svcLines,
        max_group_kVA=max_group_kVA, max_depth=max_split_depth,
        peano=peano
    )
    msg = f"Recursive split finished: {len(leaves)} leaf group(s)."
    print(msg)
    logging.info(msg)
    return leaves
...
def gui_candidate_input(G, transformerNode, meterNodes,
    ...
    if temp_root is None:
        temp_root = tk.Tk()
        temp_root.attributes('-alpha', 0.0)  # โปร่งใส แต่ยังอยู่ใน taskbar ให้ Toplevel ขึ้นได้
    ...
    if last_result is not None:
        ...
        for _btn_name in ('reopt_btn', 'recsplit_btn'):
            btn = globals().get(_btn_name, None)
            if btn is not None:
                try:
...
# ค่าเฉพาะ “Aluminium waterproof (AW)” 50/95 mm² (Ω/km) — ตามของเดิมคุณ
AW_IMP = {
    50:  {"R1": 0.552500, "X1": 0.360233, "R0": 1.153394, "X0": 0.985503},
    95:  {"R1": 0.308100, "X1": 0.340069, "R0": 0.643187, "X0": 0.930338},
}

def phase_nodes_from_designation(pd: Any):
    """
    รับได้ทั้ง:
      - เลข 7/6/5/4/3/2/1 (ตาม mapping เดิม)
      - สตริง เช่น "ABC", "AB", "A", "B", "C", "CA", "BC"
      - สตริงที่มีตัวคั่น เช่น "A,B", "A/B", "A B", "ABN" (จะดึง A/B/C ออกมา)
    คืนค่าเป็น list ของ node index ใน OpenDSS:
      A->1, B->2, C->3

    mapping เลข:
      7-ABC, 6-AB, 5-CA, 4-A, 3-BC, 2-B, 1-C
    """
    # 1) ถ้าเป็นตัวเลข/แปลงเป็นตัวเลขได้
    try:
        pd_int = int(float(pd))
        return {
            7: [1, 2, 3],   # ABC
            6: [1, 2],      # AB
            5: [3, 1],      # CA  (คงของเดิมคุณ: C,A)
            4: [1],         # A
            3: [2, 3],      # BC
            2: [2],         # B
            1: [3],         # C
    ...
    # 2) ถ้าเป็นสตริง เช่น "AB", "ABC", "A,B", "A/B", "ABN"
    s = (str(pd) if pd is not None else "").strip().upper()
    if not s:
        return [1, 2, 3]

    # ดึงเฉพาะ A/B/C ที่พบ (คงลำดับ A,B,C)
    hasA = "A" in s
    hasB = "B" in s
    hasC = "C" in s

    out = []
    if hasA: out.append(1)
    if hasB: out.append(2)
    if hasC: out.append(3)

    return out if out else [1, 2, 3]


def impedance_from_size_and_type(conductor_type: Any, conductor_size: Any) -> Tuple[float, float, float, float]:
    """
    คืนค่า (R1, X1, R0, X0) หน่วย Ω/km

    กติกา:
    - ถ้าเป็น AW และ size 50/95 -> ใช้ตาราง AW_IMP
    - ถ้าไม่เข้าเงื่อนไข -> fallback แบบสเกล R ตามขนาด (ประมาณการ)
      * R1 สเกล ~ 1/size
      * X1 คงที่ (ใกล้เคียง)
      * R0/X0 ตั้งเป็น ~3 เท่าของ positive sequence (placeholder)

    รองรับ conductor_size เป็น:
    - 50, 95, "50", "95", "50mm2", "95 sqmm", ฯลฯ
    """
    ctype = (str(conductor_type) if conductor_type is not None else "").strip().upper()

    # parse size ให้ robust
    size = None
    ...
    # normalize type: บางไฟล์อาจเป็น "AW-..." หรือ "ALAW"
    is_aw = ("AW" == ctype) or ctype.startswith("AW") or ("AW" in ctype)

    if is_aw and size in AW_IMP:
        v = AW_IMP[size]
        return v["R1"], v["X1"], v["R0"], v["X0"]

    # fallback model (เหมือนเดิมแต่ทำให้ชัด)
    # ตั้งต้นที่ 25 mm²
    base_r1_25 = 0.641   # Ω/km (placeholder)
    r1 = base_r1_25 * (25.0 / max(size, 1))
    x1 = 0.083           # Ω/km (placeholder)
    r0 = r1 * 3.0
    x0 = x1 * 3.0
    return float(r1), float(x1), float(r0), float(x0)
...
def cluster_points(points: List[Tuple[float, float]], tol: float) -> Tuple[List[int], List[Tuple[float, float]]]:
    """
    รวมจุดที่อยู่ใกล้กันภายใน tol (หน่วยเดียวกับพิกัด)
    คืนค่า:
      ids  : cluster id สำหรับแต่ละ point ตามลำดับ input
      reps : representative point ต่อ cluster (ใช้เป็นพิกัด node)

    วิธีทำ:
      - แบ่งพื้นที่เป็น grid ขนาด tol
      - สำหรับแต่ละจุด ตรวจหา cluster ใน cell รอบ ๆ (3x3) แล้วเช็คระยะจริง <= tol
      - ถ้าไม่เจอ สร้าง cluster ใหม่ โดยใช้จุดนั้นเป็น representative

    หมายเหตุ:
      - reps จะเป็น "จุดแรกที่เจอ" ของ cluster นั้น (ไม่ใช่ค่าเฉลี่ย)
      - ถ้าต้องการ representative เป็นค่าเฉลี่ย ต้องทำ post-process เพิ่มเอง
    """
    if tol <= 0:
...
# -----------------------
# PREP: build records/node_ids/... from JSON
# -----------------------
def _safe_filename(s: str, fallback="UNKNOWN_XFMR") -> str:
    s = (s or "").strip()
    if not s:
        s = fallback
...
def prepare_opendss_inputs_from_json(json_path: str, *, snap_tolerance: float, snap_map: dict):
    
    def _get_attr(f, key, default=None):
        return (f.get("attributes") or {}).get(key, default)

    def _has_point(f) -> bool:
        g = f.get("geometry") or {}
        return ("x" in g) and ("y" in g)

    def _has_paths(f) -> bool:
        g = f.get("geometry") or {}
        return isinstance(g.get("paths"), list) and len(g.get("paths")) > 0

    def _snap_xy(xy):
        # snap_map ของคุณเป็น dict {(x,y)->(x',y')}
        return snap_map.get((float(xy[0]), float(xy[1])), (float(xy[0]), float(xy[1])))

    def _point_xy(f):
        g = f.get("geometry") or {}
        return _snap_xy((float(g["x"]), float(g["y"])))

    def _endpoints_from_paths(f):
        g = f.get("geometry") or {}
        paths = g.get("paths")
        if not paths or not paths[0]:
            return None
        p = paths[0]
        p1 = _snap_xy((float(p[0][0]),  float(p[0][1])))
        p2 = _snap_xy((float(p[-1][0]), float(p[-1][1])))
        return p1, p2
    ...
    # xfmr
    xf_list = [ft for ft in features if _has_point(ft) and "XF" in str(_get_attr(ft, "TAG", "")).upper()]
    if not xf_list:
        raise RuntimeError("No transformer point found (TAG contains 'XF').")
    xfmr = xf_list[0]
    xfmr_attrs = (xfmr.get("attributes") or {})

    # LC backbone
    lc_lines = [ft for ft in features if _has_paths(ft) and "LC" in str(_get_attr(ft, "TAG", "")).upper()]
    if not lc_lines:
        raise RuntimeError("No backbone lines found (TAG contains 'LC').")

    # service lines
    service_lines = [ft for ft in features if _has_paths(ft) and int(_get_attr(ft, "SUBTYPECODE", -999) or -999) == 5]

    # meters
    load_points = [ft for ft in features if _has_point(ft) and int(_get_attr(ft, "SUBTYPECODE", -999) or -999) == 1]
    if not load_points:
        raise RuntimeError("No load points found (SUBTYPECODE=1 point).")

    # ---- build records + endpoints ----
    endpoints_list = []
    records = []
    for ft in lc_lines:
        ep = _endpoints_from_paths(ft)
        if not ep:
            continue
        p1, p2 = ep
        endpoints_list.extend([p1, p2])
        records.append((ft, p1, p2))

    if not records:
        raise RuntimeError("LC lines exist but no valid endpoints extracted.")

    # ---- cluster nodes ด้วย snap_tolerance ของคุณ ----
    node_ids, reps = cluster_points(endpoints_list, tol=float(snap_tolerance))
    reps_arr = np.array(reps, dtype=float)
    bus_of_node = {nid: f"N{nid:06d}" for nid in range(len(reps))}

    # ---- meter meta + meter_pts_arr ----
    meter_meta = []
    meter_pts = []

    for ft in load_points:
        peano = str(_get_attr(ft, "PEANO", "") or "").strip()
        if not peano:
            peano = str(_get_attr(ft, "TAG", "") or _get_attr(ft, "PEAMETER", "") or "UNKNOWN").strip()

        load_name = sanitize_for_dss_name(peano)
        meter_bus = f"M_{load_name}"

        pnode = _letter_to_node(_get_attr(ft, "PHASE"))
        if pnode is None:
            phs = phase_nodes_from_designation(_get_attr(ft, "PHASEDESIGNATION"))
            pnode = phs[0] if phs else 1

        kw = _get_attr(ft, "KWP", 0.0)
        kw = float(kw) if isinstance(kw, (int, float)) else 0.0

        pt = _point_xy(ft)  # <<< snap แล้ว
        meter_meta.append({
            "feat": ft,
            "peano": peano,
            "load_name": load_name,
    ...
    return xfmr_attrs, records, node_ids, reps_arr, bus_of_node, meter_meta, meter_pts_arr, service_lines
   
def sanitize_for_dss_name(name: str) -> str:
    name = (name or "").strip()
    if not name:
        return "UNKNOWN"
...
def _letter_to_node(letter: str):
    s = (letter or "").strip().upper()
    return {"A": 1, "B": 2, "C": 3}.get(s)

def write_dss_for_group_from_vars(
    dss_out_path: str,
    group_name: str,
    *,
    ...
    node_ids,
    reps_arr,
    bus_of_node_base: dict,
    meter_meta: list,
    meter_pts_arr,
    service_lines,

    tx_xy,
    group_peano: np.ndarray,
    group_kw: np.ndarray,
    group_new_phases: list,
):
    # --- lookups by PEANO (key หลัก) ---
    peano_list = [str(p).strip() for p in list(group_peano)]
    group_peano_set = set(peano_list)

    phase_by_peano = {peano_list[i]: str(group_new_phases[i]).strip().upper()
                      for i in range(len(peano_list))}
    kw_by_peano = {peano_list[i]: float(group_kw[i]) for i in range(len(peano_list))}

    reps_arr = np.asarray(reps_arr, dtype=float)
    meter_pts_arr = np.asarray(meter_pts_arr, dtype=float)

    def nearest_node(pt_xy):
        x, y = float(pt_xy[0]), float(pt_xy[1])
        d2 = (reps_arr[:, 0] - x) ** 2 + (reps_arr[:, 1] - y) ** 2
        i = int(np.argmin(d2))
        return i, float(math.sqrt(d2[i]))

    def nearest_meter(pt_xy):
        x, y = float(pt_xy[0]), float(pt_xy[1])
        d2 = (meter_pts_arr[:, 0] - x) ** 2 + (meter_pts_arr[:, 1] - y) ** 2
        i = int(np.argmin(d2))
        return i, float(math.sqrt(d2[i]))

    # --- transformer snap (ใช้ tx_xy ของกลุ่ม) ---
    bus_of_node = dict(bus_of_node_base)
    tx_node, tx_dist = nearest_node(tx_xy)
    bus_of_node[tx_node] = "source"

    # transformer params
    tx_tag  = str(xfmr_attrs.get("TAG", "XFM1"))
    tx_name = sanitize_for_dss_name(tx_tag) or "XFM1"
    ...
    # ------------------------------------------------------------------
    # 1) สร้าง Service (filtered) ก่อน + เก็บ target backbone nodes ของกลุ่มนี้
    # ------------------------------------------------------------------
    svc_cmds = []
    used_svc_names = set()
    target_nodes = set()  # backbone nodes ที่ service ของกลุ่มนี้ไปเกาะ
    target_nodes.add(tx_node)
    ...
    for j, f in enumerate(service_lines, start=1):
        ...
        meter_idx = None
        meter_end_is_p1 = False
        if d1 <= d2 and d1 <= svc_match:
            meter_idx = m1
            meter_end_is_p1 = True
        ...
        pe = str(m.get("peano", "")).strip()
        if pe not in group_peano_set:
            continue

        ph = phase_by_peano.get(pe, "").upper()
        meter_bus = m["meter_bus"]

        net_pt = p2 if meter_end_is_p1 else p1
        net_node, _ = nearest_node(net_pt)
        net_bus = bus_of_node[net_node]
        target_nodes.add(net_node)

        attrs = (f.get("attributes") or {})
        tag = str(attrs.get("TAG", "") or f"SVC_{j}")
        svc_name = sanitize_for_dss_name(tag) or f"SVC_{j}"
        if svc_name in used_svc_names:
            svc_name = f"{svc_name}_{j}"
        used_svc_names.add(svc_name)
        ...
        svc_len_km = 0.001  # 1 meter (link only)

        if ph == "ABC":
            for letter, node in (("A", 1), ("B", 2), ("C", 3)):
                svc_name_ph = f"{svc_name}_{letter}"
                svc_cmds.append(
                    f"New Line.{svc_name_ph} Phases=1 "
                    f"Bus1={net_bus}.{node}.0 Bus2={meter_bus}.{node}.0 "
                    f"LineCode=SVC_LINK Length={svc_len_km:.6f} Units=km"
                )
        else:
            pnode = _letter_to_node(ph) or int(m["pnode"])
            svc_cmds.append(
                f"New Line.{svc_name} Phases=1 "
                f"Bus1={net_bus}.{pnode}.0 Bus2={meter_bus}.{pnode}.0 "
                f"LineCode=SVC_LINK Length={svc_len_km:.6f} Units=km"
            )

    # ------------------------------------------------------------------
    # 2) เลือก LC เฉพาะที่ “จำเป็นสำหรับกลุ่มนี้” จาก tx_node ไปหา target_nodes
    #    ทำ BFS ครั้งเดียว แล้ว trace back เอา edges
    # ------------------------------------------------------------------
    # build adjacency: node -> list[(neighbor, edge_idx)]
    adj = {}
    ...
    parent = {tx_node: None}
    parent_edge = {}  # child_node -> edge_idx used from parent->child
    q = deque([tx_node])
    ...
    for tn in target_nodes:
        if tn not in parent:
            continue
        cur = tn
        while cur != tx_node and parent[cur] is not None:
            eidx = parent_edge[cur]
            keep_lc_idx.add(eidx)
            cur = parent[cur]

    # ------------------------------------------------------------------
    # 3) สร้าง LC commands เฉพาะ keep_lc_idx (นี่แหละ “แบ่งกลุ่มจริง”)
    # ------------------------------------------------------------------
    line_cmds = []
    used_line_names = set()

    for i, (f, _p1, _p2) in enumerate(records, start=1):
        rec0 = i - 1
        if rec0 not in keep_lc_idx:
            continue

        attrs = (f.get("attributes") or {})
        tag = str(attrs.get("TAG", "") or f"LC_{i}")
        line_name = sanitize_for_dss_name(tag) or f"LC_{i}"
        if line_name in used_line_names:
            line_name = f"{line_name}_{i}"
        used_line_names.add(line_name)
        ...
        r1, x1, r0, x0 = impedance_from_size_and_type(ctype, csize)
        ...
        line_cmds.append(
            f"New Line.{line_name} Phases={phases} Bus1={bus1} Bus2={bus2} "
            f"Length={length_km:.6f} Units=km R1={r1:.6f} X1={x1:.6f} R0={r0:.6f} X0={x0:.6f}"
        )

    # ------------------------------------------------------------------
    # 4) Loads เฉพาะกลุ่ม (เหมือนเดิม แต่ตอนนี้โครงข่าย LC ถูกกรองแล้ว)
    # ------------------------------------------------------------------
    load_cmds = []
    kw_total = 0.0
    kv_ln = float(basekv_lv) / math.sqrt(3.0)
    ...
    for m in meter_meta:
        ...
        kw = float(kw_by_peano.get(pe, m.get("kw", 0.0)))
        ph = phase_by_peano.get(pe, "").upper()
        meter_bus = m["meter_bus"]
        load_name = m["load_name"]

        kw_total += kw

        if ph == "ABC":
            load_cmds.append(
                f"New Load.{load_name} Bus1={meter_bus}.1.2.3.0 Phases=3 "
                f"kV={float(basekv_lv):.5f} kW={kw:.6f} pf={default_pf:.3f} Conn=Wye Model=1 Vminpu=0.2"
            )
        else:
            pnode = _letter_to_node(ph) or int(m["pnode"])
            load_cmds.append(
                f"New Load.{load_name} Bus1={meter_bus}.{pnode}.0 Phases=1 "
                f"kV={kv_ln:.5f} kW={kw:.6f} pf={default_pf:.3f} Conn=Wye Model=1 Vminpu=0.2"
            )
    ...
    out_lines += [
        ...
        f"new circuit.{group_name} basekv={float(basekv_hv):.3f} pu=1.0 phases=3 bus1=SourceBus Angle=0",
        "",
        f"! Group={group_name} Loads={len(load_cmds)} Service={len(svc_cmds)} LC(kept)={len(line_cmds)} SumkW={kw_total:.3f}",
        f"! Transformer TAG={tx_tag} snapped-to-node dist={tx_dist:.3f}",
        "",
        ...
        f"~ wdg=2 bus=source.1.2.3.0 conn=wye kv={float(basekv_lv):.3f} kva={rate_kva:.2f} %r=0.5",
        "! --- Service link linecode (topology only) ---",
        "New LineCode.SVC_LINK nphases=1 r1=0.000001 x1=0.000001 r0=0.000003 x0=0.000003 units=km",
        "! --- LV backbone (filtered by group) ---",
    ]
    out_lines += line_cmds
    out_lines += ["", "! --- Service drops (filtered) ---"]
    out_lines += svc_cmds
    out_lines += ["", "! --- Loads (filtered) ---"]
    out_lines += load_cmds
    out_lines += [
        "",
        f"Set Voltagebases=[{float(basekv_hv):.3f} {float(basekv_lv):.3f}]",
        "CalcVoltageBases",
        "",
        "Solve",
        "",
...
def main():
    ...
    global OUT_DIR, _EDGE_DF_CACHE
    
    logging.info("Program started with coordinate snapping.")
    ap = argparse.ArgumentParser(...)
    # ap.add_argument(...) ทั้งหมด
    args = ap.parse_args()

             
    # ====== JSON-based EXTRACT (drop-in) =================================
    # Use the global out_json set by __main__ (via --fac or --json); fall back to hardcoded path only
    # when main() is invoked directly (e.g. from a REPL or old entry-point).
    global out_json
    if not out_json or not os.path.exists(out_json):
        out_json = r"D:\testpy\NetworkLV55-930097_with_MV.json"

    json_path = out_json  # หรือกำหนดเป็นสตริงพาธไฟล์ของคุณเอง

    # ----- กำหนด output folder แยกตาม FACILITYID / JSON name -----
    import re as _re
    _json_base = os.path.splitext(os.path.basename(json_path))[0]
    _fac_match = _re.search(r'\d{2,3}-\d{5,7}', _json_base)
    _out_name = _fac_match.group(0) if _fac_match else _json_base
    if _API_PROJECT_ID is not None:
        # API mode: ใช้ pea_no_projects/output/{project_id}/downloads/ สำหรับ downloads
        OUT_DIR = os.path.join("pea_no_projects", "output", _API_PROJECT_ID, "downloads")
    else:
        OUT_DIR = os.path.join("output", _out_name)
    ...
    if not IS_GUI:
        try:
            ...
            log_path = os.path.join(OUT_DIR, f"{json_name}.run.log")
    ...
    logging.info("Building initial network with snapping …")   
        ...
        # 1) สร้าง coord_snap_map จาก LV+MV+svcLines ด้วย SNAP_TOLERANCE
        # ต้องรวม svcLines ด้วย เพราะ endpoint ของ service line อาจต่างจาก LV node แค่ 0.001m
        # ถ้าไม่รวม → สร้าง node แยก → meter isolated component
    all_line_coords = []
    for L in (lvLines, mvLines, svcLines):
        for line in L:
    ...
    max_dist_m, i_far, node_far, xy_far = farthest_meter_from_initial_transformer_log(
    G_init, tNode_init, mNodes_init, meterLocations=meterLocations,peano=peano
)

    # Compute original system metrics (before improvement) for results.json
    _total_load_kW_orig = float((phase_loads['A'] + phase_loads['B'] + phase_loads['C']).sum())
    try:
        _, _, _loss_W_orig = calculateUnbalancedPowerFlow(
            G_init, tNode_init, mNodes_init, powerFactor, initialVoltage
        )
        _line_loss_orig_kW = _loss_W_orig / 1000.0
        _tx_loss_orig_kW = get_transformer_losses(_total_load_kW_orig)
        total_loss_before = round(_line_loss_orig_kW + _tx_loss_orig_kW, 2)
    except Exception as _e:
        logging.warning(f"Could not compute original system loss: {_e}")
        total_loss_before = 0.0
    pload_peak_pct = round(
        _total_load_kW_orig / (transformerCapacity_kVA * powerFactor) * 100, 1
    ) if transformerCapacity_kVA > 0 else 0.0

    # For this run we use LV-based optimization
    optimizedTransformerLocation_LV = optimizeTransformerLocationOnLVCond3(
        meterLocations, phase_loads, tx_xy,
        lvLines, initialVoltage, conductorResistance, powerFactor,
        conductorReactance=conductorReactance,
        lvData=lvData,
        svcLines=svcLines
    ...
    reconnect_edges = [
        (a, b) for a, b, d in G_for_split.edges(data=True)
        if d.get("tx_component_reconnect", False)
    ]

    if reconnect_edges:
        G_for_split.remove_edges_from(reconnect_edges)
        logging.warning(
            "Removed %d tx_component_reconnect edges before split/partition.",
            len(reconnect_edges)
        )

    splitting_edge, sp_coord, sp_edge_diff, candidate_edges = findSplittingPoint(
        G_for_split, transformerNode, meterNodes, coord_mapping,
        powerFactor, initialVoltage, candidate_index=_API_SP_INDEX
    )
    # บันทึก cache ทันที ก่อน optimizeGroup จะ overwrite ด้วย inner findSplittingPoint
    _main_edge_df_cache = _EDGE_DF_CACHE

    if splitting_edge is None:
        logging.warning("No splitting edge found; single group only. End.")
        return
        
    group1_nodes, group2_nodes = partitionNetworkAtPoint(G_for_split, transformerNode, meterNodes, splitting_edge, coord_mapping=coord_mapping)

    allowed_g1_coords = get_allowed_candidate_coords_after_split(G_for_split,coord_mapping,splitting_edge,group1_nodes,meterNodes)
    allowed_g2_coords = get_allowed_candidate_coords_after_split(G_for_split,coord_mapping,splitting_edge,group2_nodes,meterNodes)
    # group1_nodes/group2_nodes ถูก partition จาก G_for_split แล้วข้างบน
    # ใช้ FBS บน G เพื่อคำนวณแรงดัน/กระแสเท่านั้น ไม่เขียนทับ partition ที่ถูกต้อง
    voltages, branch_curr, _g1_fbs, _g2_fbs = performForwardBackwardSweepAndDivideLoads(
    ...
    tag_prefix_for_lv=("LC",),  # substring match: ครอบ TAG รูปแบบ "22LCEA...", "2233LC...", "2244LC..." ฯลฯ (office/area code นำหน้าไม่คงที่)
    subtype_allow=(1,),
    opvolt_max=1000.0
    )
    ...
    # ใช้ lvData จริงเพื่อให้ optimizeGroup ไล่ PHASEDESIGNATION 7->1 ได้
    logging.info("Optimizing Transformer Location each Group...")
    optimizedTransformerLocationGroup1 = optimizeGroup(
        ...
        allowed_candidate_coords=allowed_g1_coords  # << เฉพาะกลุ่ม 1
    )
    optimizedTransformerLocationGroup2 = optimizeGroup(
        ...
        svcLines=svcLines,
        use_phase_filter=False,
        allowed_candidate_coords=allowed_g2_coords  # << เฉพาะกลุ่ม 2
    )
    ...
    if optimizedTransformerLocationGroup1 is not None:
        ...
        tx_xy_g1 = np.array(cm_g1.get(tNode_g1, [np.nan, np.nan]), dtype=float)
        dist_g1 = []
        for i, mNode in enumerate(mNodes_g1):
            try:
                dist_g1.append(nx.shortest_path_length(G_g1, tNode_g1, mNode, weight='weight'))
            except nx.NetworkXNoPath:
                # Meter disconnected from TR (service line gap); fallback to Euclidean distance
                m_xy = np.array(cm_g1.get(mNode, [np.nan, np.nan]), dtype=float)
                dist_g1.append(float(np.hypot(m_xy[0] - tx_xy_g1[0], m_xy[1] - tx_xy_g1[1])))
        ...
        # ---- Voltage AFTER phase balance (TX location เดิม, สลับโหลดเป็น new_phase_loads_g1) ----
        G_g1_bal, tNode_g1_bal, mNodes_g1_bal, _, _ = buildLVNetworkWithLoads(
            lvLines, mvLines,
            group1_meterLocs,
            optimizedTransformerLocationGroup1,
            new_phase_loads_g1,
        ...
        node_voltages_g1_bal, _, total_power_loss_g1_bal = calculateUnbalancedPowerFlow(
            G_g1_bal, tNode_g1_bal, mNodes_g1_bal, powerFactor, initialVoltage
        )

        worst_before_g1, worst_after_g1 = [], []
        for i, node_bal in enumerate(mNodes_g1_bal):
            global_idx = g1_idx[i]
            vA = abs(node_voltages_g1_bal[node_bal]['A'])
            vB = abs(node_voltages_g1_bal[node_bal]['B'])
            vC = abs(node_voltages_g1_bal[node_bal]['C'])
            result_df.at[global_idx, 'Voltage A After Balance (V)'] = vA
            result_df.at[global_idx, 'Voltage B After Balance (V)'] = vB
            result_df.at[global_idx, 'Voltage C After Balance (V)'] = vC
            worst_after_g1.append(min(vA, vB, vC))
        for i, node in enumerate(mNodes_g1):
            worst_before_g1.append(min(
                abs(node_voltages_g1[node]['A']),
                abs(node_voltages_g1[node]['B']),
                abs(node_voltages_g1[node]['C'])
            ))

        if worst_before_g1:
            logging.info(
                "Group 1 worst-phase voltage per meter: BEFORE balance min=%.2f V, mean=%.2f V | "
                "AFTER balance min=%.2f V, mean=%.2f V | Δmin=%+.2f V, Δmean=%+.2f V",
                min(worst_before_g1), float(np.mean(worst_before_g1)),
                min(worst_after_g1), float(np.mean(worst_after_g1)),
                min(worst_after_g1) - min(worst_before_g1),
                float(np.mean(worst_after_g1)) - float(np.mean(worst_before_g1))
            )
            logging.info(
                "Group 1 line loss: BEFORE balance %.2f kW -> AFTER balance %.2f kW",
                total_power_loss_g1 / 1000.0, total_power_loss_g1_bal / 1000.0
            )
    ...
    if optimizedTransformerLocationGroup2 is not None:
        ...
        tx_xy_g2 = np.array(cm_g2.get(tNode_g2, [np.nan, np.nan]), dtype=float)
        dist_g2 = []
        for i, mNode in enumerate(mNodes_g2):
            try:
                ...
                # Meter disconnected from TR (service line gap); fallback to Euclidean distance
                m_xy = np.array(cm_g2.get(mNode, [np.nan, np.nan]), dtype=float)
                dist_g2.append(float(np.hypot(m_xy[0] - tx_xy_g2[0], m_xy[1] - tx_xy_g2[1])))
        ...
        # ---- Voltage AFTER phase balance (TX location เดิม, สลับโหลดเป็น new_phase_loads_g2) ----
        G_g2_bal, tNode_g2_bal, mNodes_g2_bal, _, _ = buildLVNetworkWithLoads(
            lvLines, mvLines,
            group2_meterLocs,
            optimizedTransformerLocationGroup2,
            new_phase_loads_g2,
        ...
        node_voltages_g2_bal, _, total_power_loss_g2_bal = calculateUnbalancedPowerFlow(
            G_g2_bal, tNode_g2_bal, mNodes_g2_bal, powerFactor, initialVoltage
        )

        worst_before_g2, worst_after_g2 = [], []
        for i, node_bal in enumerate(mNodes_g2_bal):
            global_idx = g2_idx[i]
            vA = abs(node_voltages_g2_bal[node_bal]['A'])
            vB = abs(node_voltages_g2_bal[node_bal]['B'])
            vC = abs(node_voltages_g2_bal[node_bal]['C'])
            result_df.at[global_idx, 'Voltage A After Balance (V)'] = vA
            result_df.at[global_idx, 'Voltage B After Balance (V)'] = vB
            result_df.at[global_idx, 'Voltage C After Balance (V)'] = vC
            worst_after_g2.append(min(vA, vB, vC))
        for i, node in enumerate(mNodes_g2):
            worst_before_g2.append(min(
                abs(node_voltages_g2[node]['A']),
                abs(node_voltages_g2[node]['B']),
                abs(node_voltages_g2[node]['C'])
            ))

        if worst_before_g2:
            logging.info(
                "Group 2 worst-phase voltage per meter: BEFORE balance min=%.2f V, mean=%.2f V | "
                "AFTER balance min=%.2f V, mean=%.2f V | Δmin=%+.2f V, Δmean=%+.2f V",
                min(worst_before_g2), float(np.mean(worst_before_g2)),
                min(worst_after_g2), float(np.mean(worst_after_g2)),
                min(worst_after_g2) - min(worst_before_g2),
                float(np.mean(worst_after_g2)) - float(np.mean(worst_before_g2))
            )
            logging.info(
                "Group 2 line loss: BEFORE balance %.2f kW -> AFTER balance %.2f kW",
                total_power_loss_g2 / 1000.0, total_power_loss_g2_bal / 1000.0
            )
    ...
    # เตรียมข้อมูล common สำหรับเขียน DSS

    xfmr_attrs, records, node_ids, reps_arr, bus_of_node, meter_meta, meter_pts_arr, service_lines = \
    prepare_opendss_inputs_from_json(
        json_path,
        snap_tolerance=_snap_lv,
        snap_map=_snap_map_lv
    )    
        # ดึง facilityid (เผื่อบางไฟล์ใช้ชื่อ field ต่างกัน)
    facilityid = (
        xfmr_attrs.get("FACILITYID")
        or xfmr_attrs.get("FacilityID")
    )

    base = _safe_filename(str(facilityid))

    out_g1 = os.path.join(OUT_DIR, f"{base}_G1.dss")
    out_g2 = os.path.join(OUT_DIR, f"{base}_G2.dss")
    
    
    # แล้วค่อยเรียก write_dss_for_group_from_vars(...)
    write_dss_for_group_from_vars(
        dss_out_path= out_g1,
        group_name="G1",
        basekv_hv=getattr(args, "basekv_hv", 22.0),
        basekv_lv=getattr(args, "basekv_lv", 0.4),
        default_pf=getattr(args, "default_pf", 0.875),
        svc_match=getattr(args, "svc_match", 20.0),
        xfmr_attrs=xfmr_attrs,
        records=records,
        node_ids=node_ids,
        reps_arr=reps_arr,
        bus_of_node_base=bus_of_node,
        meter_meta=meter_meta,
        meter_pts_arr=meter_pts_arr,
        service_lines=service_lines,
        tx_xy=optimizedTransformerLocationGroup1,
        group_peano=peano_g1,
        group_kw=loads_g1,
        group_new_phases=new_phases_g1,
    )
    write_dss_for_group_from_vars(
        dss_out_path=out_g2,
        group_name="G2",
        ...
        meter_pts_arr=meter_pts_arr,
        service_lines=service_lines,
        tx_xy=optimizedTransformerLocationGroup2,
        group_peano=peano_g2,
        group_kw=loads_g2,
        group_new_phases=new_phases_g2,
    )
    ...
    if IS_GUI:
        ...
        # Headless/API mode: เขียน results.json + geojson สำหรับ web app
        if _API_PROJECT_ID is not None:
            _project_out = os.path.join("pea_no_projects", "output", _API_PROJECT_ID)
            os.makedirs(_project_out, exist_ok=True)
            _results = {
                "sp_index": _API_SP_INDEX,
                "tr_pea_no": str(facilityid) if facilityid else "",
                "tr_kva": round(float(transformerCapacity_kVA), 2),
                "tr_loss": total_loss_before,
                "tr_Line_lengh": round(float(max_dist_m), 1),
                "tr_PLOADPEAK": pload_peak_pct,
                "g1_load": round(float(g1_load), 2),
                "future_g1_load": round(float(future_g1_load), 2),
                "rating_g1": rating_g1,
                "line_loss_g1_kW": round(float(line_loss_g1_kW), 2),
                "tx_loss_g1_kW": round(float(tx_loss_g1_kW), 2),
                "total_system_loss_g1": round(float(total_system_loss_g1), 2),
                "g2_load": round(float(g2_load), 2),
                "future_g2_load": round(float(future_g2_load), 2),
                "rating_g2": rating_g2,
                "line_loss_g2_kW": round(float(line_loss_g2_kW), 2),
                "tx_loss_g2_kW": round(float(tx_loss_g2_kW), 2),
                "total_system_loss_g2": round(float(total_system_loss_g2), 2),
                "Group_1_percent_unbalance_before": round(float(g1_unb_before), 1),
                "Group_1_percent_unbalance_after": round(float(g1_unb_after), 1),
                "Group_2_percent_unbalance_before": round(float(g2_unb_before), 1),
                "Group_2_percent_unbalance_after": round(float(g2_unb_after), 1),
                "Max_Distance_Group1": round(float(max_dist1), 1),
                "Max_Distance_Group2": round(float(max_dist2), 1),
                "group1_load_balance_before": {
                    "A": round(float(group1_phase_loads['A'].sum()), 1),
                    "B": round(float(group1_phase_loads['B'].sum()), 1),
                    "C": round(float(group1_phase_loads['C'].sum()), 1),
                },
                "group1_load_balance_after": {
                    "A": round(float(new_phase_loads_g1['A'].sum()), 1),
                    "B": round(float(new_phase_loads_g1['B'].sum()), 1),
                    "C": round(float(new_phase_loads_g1['C'].sum()), 1),
                },
                "group2_load_balance_before": {
                    "A": round(float(group2_phase_loads['A'].sum()), 1),
                    "B": round(float(group2_phase_loads['B'].sum()), 1),
                    "C": round(float(group2_phase_loads['C'].sum()), 1),
                },
                "group2_load_balance_after": {
                    "A": round(float(new_phase_loads_g2['A'].sum()), 1),
                    "B": round(float(new_phase_loads_g2['B'].sum()), 1),
                    "C": round(float(new_phase_loads_g2['C'].sum()), 1),
                },
            }
            with open(os.path.join(_project_out, "results.json"), "w", encoding="utf-8") as _f:
                json.dump(_results, _f, ensure_ascii=False, indent=2)
            logging.info(f"[API] results.json saved: {_project_out}/results.json")
            _cache_dir = os.path.join(_project_out, "results_cache")
            os.makedirs(_cache_dir, exist_ok=True)
            with open(os.path.join(_cache_dir, f"{_API_SP_INDEX}.json"), "w", encoding="utf-8") as _f:
                json.dump(_results, _f, ensure_ascii=False, indent=2)
            logging.info(f"[API] results cached for sp_index={_API_SP_INDEX}")
            _write_headless_geojson(
                _project_out, lvLines, mvLines, filteredEserviceLines,
                meterLocations, g1_plot_indices, g2_plot_indices, phases, result_df,
                tx_xy, sp_coord, optimizedTransformerLocationGroup1, optimizedTransformerLocationGroup2
            )
            # เก็บสำเนา geojson แยกตาม sp_index ไว้ด้วย เพื่อให้หน้าแผนที่สลับดูผลลัพธ์ที่ cache ไว้
            # ได้โดยไม่ต้อง reprocess ใหม่ (meter_groups/feature_groups เปลี่ยนตาม splitting index ที่เลือก)
            _write_headless_geojson(
                os.path.join(_cache_dir, str(_API_SP_INDEX)),
                lvLines, mvLines, filteredEserviceLines,
                meterLocations, g1_plot_indices, g2_plot_indices, phases, result_df,
                tx_xy, sp_coord, optimizedTransformerLocationGroup1, optimizedTransformerLocationGroup2
            )

            # เขียน edge_diffs.json สำหรับ index selector บนแผนที่
            # ใช้ _main_edge_df_cache (บันทึกไว้ก่อน optimizeGroup จะ overwrite ด้วย inner
            # findSplittingPoint) แทน _EDGE_DF_CACHE สดที่อาจถูกเขียนทับไปแล้ว ณ จุดนี้
            if _main_edge_df_cache is not None and not _main_edge_df_cache.empty:
                try:
                    _proj_tm3   = pyproj.CRS.from_proj4("+proj=utm +zone=47 +datum=WGS84 +units=m +no_defs")
                    _proj_wgs84 = pyproj.CRS("EPSG:4326")
                    _xf_ed = pyproj.Transformer.from_crs(_proj_tm3, _proj_wgs84, always_xy=True)
                    _ed_df = _main_edge_df_cache.copy()
                    _ed_df["N1_Lon"], _ed_df["N1_Lat"] = _xf_ed.transform(_ed_df["N1_X"].values, _ed_df["N1_Y"].values)
                    _ed_df["N2_Lon"], _ed_df["N2_Lat"] = _xf_ed.transform(_ed_df["N2_X"].values, _ed_df["N2_Y"].values)
                    _ed_json_df = _ed_df.drop(columns=[c for c in ["N1_X","N1_Y","N2_X","N2_Y"] if c in _ed_df.columns])
                    _edge_diffs_path = os.path.join(_project_out, "edge_diffs.json")
                    with open(_edge_diffs_path, "w", encoding="utf-8") as _f:
                        json.dump(_ed_json_df.to_dict(orient="records"), _f, ensure_ascii=False, indent=2)
                    logging.info(f"[API] edge_diffs.json saved: {_edge_diffs_path} ({len(_ed_json_df)} entries)")
                except Exception as _ed_err:
                    logging.warning(f"[API] Could not write edge_diffs.json: {_ed_err}")
    ...
    if IS_GUI:
        _EDGE_DF_CACHE = _main_edge_df_cache  # restore cache ที่ถูก overwrite โดย optimizeGroup
...
def _write_headless_geojson(output_dir, lvLines, mvLines, eserviceLines,
                             meterLocations, g1_indices, g2_indices, phases, result_df,
                             tx_xy, sp_coord, tx_g1, tx_g2):
    """เขียน GeoJSON ที่ web app ต้องการสำหรับหน้าแผนที่ (headless/API mode)"""
    try:
        _proj_tm3 = pyproj.CRS.from_proj4("+proj=utm +zone=47 +datum=WGS84 +units=m +no_defs")
        _proj_wgs84 = pyproj.CRS("EPSG:4326")
        _xf = pyproj.Transformer.from_crs(_proj_tm3, _proj_wgs84, always_xy=True)
    except Exception as e:
        logging.error(f"[headless] pyproj init failed: {e}")
        return

    def _to_latlon(x, y):
        return _xf.transform(x, y)
    ...
    lv_feats = []
    for line in lvLines:
        coords = [_to_latlon(x, y) for x, y in zip(line['X'], line['Y'])]
        lv_feats.append({"type": "Feature", "geometry": {"type": "LineString", "coordinates": coords},
                         "properties": {"type": "LV Line"}})
    with open(os.path.join(output_dir, "lv_lines.geojson"), "w", encoding="utf-8") as _f:
        json.dump({"type": "FeatureCollection", "features": lv_feats}, _f, indent=2)

    # MV lines
    mv_feats = []
    for line in mvLines:
        coords = [_to_latlon(x, y) for x, y in zip(line['X'], line['Y'])]
        mv_feats.append({"type": "Feature", "geometry": {"type": "LineString", "coordinates": coords},
                         "properties": {"type": "MV Line"}})
    with open(os.path.join(output_dir, "mv_lines.geojson"), "w", encoding="utf-8") as _f:
        json.dump({"type": "FeatureCollection", "features": mv_feats}, _f, indent=2)

    # Meter groups
    meter_feats = []
    _g1_set = set(g1_indices.tolist()) if hasattr(g1_indices, 'tolist') else set(g1_indices)
    _g2_set = set(g2_indices.tolist()) if hasattr(g2_indices, 'tolist') else set(g2_indices)
    for idx in _g1_set | _g2_set:
        x, y = meterLocations[idx]
        lon, lat = _to_latlon(x, y)
        ph = str(phases[idx]).upper().strip() if phases is not None and idx < len(phases) else ""
        vtext = ""
        if result_df is not None:
            for ph_char in ['A', 'B', 'C']:
                col = f'Final Voltage {ph_char} (V)'
                if ph_char in ph and col in result_df.columns:
                    try:
                        vv = result_df.iloc[idx][col]
                        if not (vv != vv):  # not NaN
                            vtext += f"{ph_char}:{vv:.1f}V\n"
                    except Exception:
                        pass
        meter_feats.append({
            "type": "Feature",
            "geometry": {"type": "Point", "coordinates": [lon, lat]},
            "properties": {"group": 1 if idx in _g1_set else 2, "voltage_text": vtext.strip()}
        })
    with open(os.path.join(output_dir, "meter_groups.geojson"), "w", encoding="utf-8") as _f:
        json.dump({"type": "FeatureCollection", "features": meter_feats}, _f, ensure_ascii=False, indent=2)

    # Feature groups (TR locations, splitting point)
    feat_items = []
    if tx_xy is not None:
        lon, lat = _to_latlon(tx_xy[0], tx_xy[1])
        feat_items.append(Feature(geometry=Point((lon, lat)), properties={"name": "Initial Transformer", "group": "initial"}))
    if sp_coord is not None:
        lon, lat = _to_latlon(sp_coord[0], sp_coord[1])
        feat_items.append(Feature(geometry=Point((lon, lat)), properties={"name": "Splitting Point", "group": "splitting"}))
    if tx_g1 is not None:
        lon, lat = _to_latlon(tx_g1[0], tx_g1[1])
        feat_items.append(Feature(geometry=Point((lon, lat)), properties={"name": "Group 1 Transformer", "group": "group1"}))
    if tx_g2 is not None:
        lon, lat = _to_latlon(tx_g2[0], tx_g2[1])
        feat_items.append(Feature(geometry=Point((lon, lat)), properties={"name": "Group 2 Transformer", "group": "group2"}))
    with open(os.path.join(output_dir, "feature_groups.geojson"), "w", encoding="utf-8") as _f:
        json.dump(FeatureCollection(feat_items), _f, ensure_ascii=False, indent=2)
...
def createGUI():
    ...
    global root, transformerFileName, reopt_btn, recsplit_btn, lvData, conductorReactance
    ...
    # Add the "Split Subgroups (Recursive)" Button
    recsplit_btn = tk.Button(top_frame, text="Split Subgroups (Recursive)",
                             state=tk.DISABLED,   # disabled until we have data
                             command=runRecursiveSplit)
    recsplit_btn.pack(side=tk.LEFT, padx=5)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--recursive-split", action="store_true",
                        help="แบ่งกลุ่ม 1/2 ที่ได้ต่อไปเรื่อยๆ (recursive) จนโหลดแต่ละกลุ่ม <= --max-group-kva")
    parser.add_argument("--max-group-kva", type=float, default=RECURSIVE_SPLIT_MAX_KVA,
                        help=f"เพดานโหลดต่อกลุ่มใบ (kVA) ตอนใช้ --recursive-split (ค่าเริ่มต้น {RECURSIVE_SPLIT_MAX_KVA})")
    parser.add_argument("--max-split-depth", type=int, default=RECURSIVE_SPLIT_MAX_DEPTH,
                        help=f"safety valve จำกัดความลึกของ recursive split (ค่าเริ่มต้น {RECURSIVE_SPLIT_MAX_DEPTH})")
    args = parser.parse_args()

    # หลัง parser.parse_args()
    IS_GUI = bool(getattr(args, "gui", False))
    RECURSIVE_SPLIT_ENABLED = bool(getattr(args, "recursive_split", False))
    RECURSIVE_SPLIT_MAX_KVA = float(getattr(args, "max_group_kva", RECURSIVE_SPLIT_MAX_KVA))
    RECURSIVE_SPLIT_MAX_DEPTH = int(getattr(args, "max_split_depth", RECURSIVE_SPLIT_MAX_DEPTH))
    ...
    # ---------- 2.5) interactive prompt เมื่อไม่มี input และไม่ใช่ GUI ----------
    _interactive = False
    if not args.gui and not fac and not jpth:
        print("=" * 50)
        print("  Transformer Optimization — Interactive Mode")
        print("=" * 50)
        print("กรอก FACILITYID หม้อแปลง  เช่น  27-007795")
        print("หรือกรอก path ไฟล์ JSON   เช่น  NetworkLV27-007795_with_MV.json")
        print("(กด Enter โดยไม่กรอกอะไรเพื่อออก)")
        print("-" * 50)
        _user_input = input("FACILITYID / JSON path: ").strip()
        if not _user_input:
            print("ไม่มีอินพุต ออกจากโปรแกรม")
            sys.exit(0)
        # ถ้าดูเหมือน path ไฟล์ → ใช้เป็น --json, ไม่งั้นถือว่าเป็น FACILITYID
        if _user_input.endswith(".json") or os.sep in _user_input or "/" in _user_input:
            jpth = _user_input
        else:
            fac = _user_input
        _interactive = True  # บังคับ verbose เพื่อให้เห็น progress ใน terminal

    # ---------- 3) setup logging รอบแรก (ชื่อกลาง ๆ แยกตาม PID กัน lock) ----------
    default_log_dir = os.path.join(os.getcwd(), "logs")
    _init_log_name = f"run_{os.getpid()}.log" if (fac or jpth) else "run.run.log"
    log_path = os.path.join(default_log_dir, _init_log_name)

    logger = setup_logging(
        log_path,
        to_console=getattr(args, "verbose", False) or _interactive,
        level=logging.INFO,
