...
> **ขั้นตอนมาตรฐานประจำวัน (โฟลเดอร์ `incoming/`, สัญญาที่ต้องรักษา, สคริปต์ตรวจ) ดูที่ [`NEW_CODE_INTEGRATION_GUIDE.md`](NEW_CODE_INTEGRATION_GUIDE.md)** — เอกสารนี้เก็บ convention และกรณีศึกษาแต่ละรอบ
...
## กรณีศึกษาจริง (2026-09-18 ต่อ): บั๊ก 2 ตัวที่เจอจากการไล่ debug "มิเตอร์ไม่ขึ้น map"

ผู้ใช้แจ้งว่ามิเตอร์บางส่วนไม่แสดงบน `/phase_optimizer/map/` แม้ backend ดูเหมือนจะสร้างข้อมูลถูกต้อง (ตรวจสอบซ้ำหลายรอบด้วยการรันจริงผ่าน `run_web.py` กับ facility ของผู้ใช้ตรงๆ ไม่ใช่แค่ตรวจโค้ดเฉยๆ) — พบ 2 บั๊กแยกกันคนละชั้น ทั้งคู่ยืนยันด้วยการรันจริงก่อนสรุปว่าคือสาเหตุ:

**1) Frontend — บั๊ก `MapView.goTo()` ของ ArcGIS Maps SDK for JavaScript 4.29**: error จริงจาก browser console คือ `Uncaught (in promise) TypeError: Cannot read properties of undefined (reading 'animation')` ตรงกับ breaking change ที่มีรายงานใน Esri Community สำหรับ 4.29 โดยเฉพาะ (คำนวณ animation/rotation ของ extent พัง) — หน้า `phase_optimizer_map.html` เรียก `view.goTo()` แบบ auto-pan ไปหม้อแปลงตอนโหลดหน้า และใน `panToFeature()` (คลิกรายการจุดสำคัญ) โดยไม่มี error handling เลย พอชนบั๊กกลางแอนิเมชัน view จะค้างสถานะเสียจนเลเยอร์อื่นไม่โหลดฟีเจอร์เพิ่มอีกแม้จะซูม/เลื่อนต่อ — แก้โดยเพิ่ม `{animate: false}` ให้ทุกจุดที่เรียก `goTo()` (ข้ามเส้นทางโค้ดที่มีบั๊กไปเลย โดยไม่ต้องรอ Esri แก้หรือขยับเวอร์ชัน SDK ซึ่งเสี่ยง breaking change อื่น) พร้อมเพิ่ม `.catch(() => {})` ทุก promise chain ที่ไม่มี catch มาก่อน กันไม่ให้ error อื่นในอนาคตทำลายสถานะ view ซ้ำ

**2) Backend — `select_load_point_indices()` กรองมิเตอร์ 3 เฟสทิ้งทั้งหมด (บั๊กจริง ไม่ใช่แค่ frontend)**: หลัง fix ข้อ 1 ผู้ใช้ยังแจ้งว่า "มิเตอร์แสดงไม่หมด" — ไล่เทียบจำนวน feature ในไฟล์ JSON ดิบ (นับจาก point feature ที่มี `PEANO`/`PEAMETER`) กับจำนวนที่ผ่านเข้า `net.load_feat_nodes` แล้วเจอมิเตอร์หายไปจริง 1-2 ตัวต่อ facility ที่ทดสอบ ตรวจ attribute แล้วพบว่าเป็นมิเตอร์ **3 เฟส** จริง (`PHASE="ABC"`, `PHASEDESIGNATION=7`, มี `KWP` โหลดจริง) แต่ `SUBTYPECODE=3` แทนที่จะเป็น `1` — ฟังก์ชัน `select_load_point_indices()` ใน `Runopendss_All18092026.py` (ใช้ร่วมกันทั้ง `convert_json_to_dss_ordered()` และ `TransferOptimizer_16092026.NetworkGraph`) เดิมรับเฉพาะ `SUBTYPECODE==1` เท่านั้น จึงกรองมิเตอร์ 3 เฟสทิ้งไป**ทั้งจากแผนที่และจากวงจร OpenDSS** (โหลดของมิเตอร์เหล่านี้หายไปจากผลจำลองไฟฟ้าทั้งก้อน ไม่ใช่แค่ไม่แสดงผลบนแผนที่) — บั๊กนี้มีมาตั้งแต่ฟังก์ชันนี้ถูกสร้างในรอบ 16092026 ไม่เกี่ยวกับการแปลง API เลย

แก้โดยขยาย filter เป็น `subtype not in (1, 3): continue` — guard อื่นที่มีอยู่แล้ว (TAG มี "XF" → ข้าม, ต้องมี PEANO/PEAMETER) เพียงพอกันหม้อแปลงเอง (ซึ่งก็มี `SUBTYPECODE=3` เหมือนกันแต่ TAG มีคำว่า "XF") ไม่ให้หลุดเข้ามาเป็นมิเตอร์ปลอม — ตรวจสอบด้วยการรันจริงทั้ง 2 facility ซ้ำ: `54-008953` มิเตอร์เพิ่มจาก 234→235 (ตัวที่หายไปกลับมาแสดงเป็นเฟส A ตามจุดต่อจริงบน backbone แม้ tag ดิบจะบอกว่า ABC ก็ตาม — ยืนยันว่า `_meter_current_pd()` ใช้เฟสจริงจากจุดต่อ ไม่ใช่ tag ดิบ ปลอดภัย), `59-018889` เพิ่มจาก 194→196 (นับ TR-placeholder ที่ไม่ควรนับ ยังถูกกรองออกถูกต้อง)

พร้อมกันนี้ปรับความหนาเส้น "ช่วงเพิ่มขนาดสาย"/"ช่วงเพิ่มเฟสสาย" บนแผนที่จาก width 4 → 2 ตามที่ขอ

## กรณีศึกษาจริง (2026-09-22): แปลง `PhaseOptimizer_22092026.py` (รอบแรกที่ใช้ `incoming/`)

วางที่ `incoming/` → ผนวกตาม [`NEW_CODE_INTEGRATION_GUIDE.md`](NEW_CODE_INTEGRATION_GUIDE.md) ทุกขั้น

**scaffolding หายซ้ำชุดเดิมครบ** (sys.path bootstrap, `Runopendss_All05082026`/`from TransferOptimizer import` ตรงๆ, `DEFAULT_MAX_IMBALANCE_PCT` กลับเป็น 20.0, `region`, `self.error` ทั้ง 2 จุดใน `run()`, `_ensure_json()` กลับไปเรียก `INPUT_FACILITY`, `--region`) — คืนครบ และคงเกณฑ์ **25%** ตามที่ผู้ใช้ยืนยันไว้ (ไม่ถามซ้ำ) diff ที่เหลือ 13 hunks เป็น logic ใหม่ล้วน

Logic ใหม่จริงที่คงไว้ทั้งหมด (ปรับให้ Design Check เพิ่มเฟสแล้ว "ใช้เฟสใหม่จริง" และงานหน้างานน้อยลง):
- `DESIGN_MIN_METERS_ABS = 3`: กลุ่มสายที่มีมิเตอร์ < 3 ตัวไม่สั่งเพิ่มเฟสเลย แม้สายจะยาว
- `MIN_MOVE_GAIN_KW` 0.02 → 0.10 (วัดจาก 100 หม้อแปลง: ย้ายลดจาก 675 → 566 ตัว สมดุลเท่าเดิม)
- `_DESIGN_TARGET_FRAC = 0.9` (เป้าภายใน = 0.9 × เกณฑ์ แทน เกณฑ์ − margin), `_POOL_DEV_TARGET_PCT = 18.0` (เกณฑ์ต่อกิ่ง), `_NEW_PHASE_MIN_SHARE = 0.20` (เฟสที่เพิ่งเดินเพิ่มต้องรับโหลดอย่างน้อย 20% ของกิ่ง) และเกลี่ย **ทุกกิ่ง** ที่เพิ่มเฟส ไม่หยุดกลางคันเมื่อหม้อแปลงผ่านแล้ว
- `design_pa_rejected`: ถ้า Step 3c เสนอเพิ่มเฟสแต่ไม่ผ่านเกณฑ์ (พิกัดเฟสเกิน/แรงดันต่ำ/Unbalance แย่ลง) จะพิมพ์เหตุผลและไม่จำลองซ้ำใน Step 6
- หมายเหตุ: ค่าจูนข้างบนวัดโดยผู้พัฒนาบนเกณฑ์ 20% (comment ใน raw เขียน "0.9 × 20% = 18%") ระบบเราใช้ 25% จึงเป้าภายใน = 22.5% — พฤติกรรมตามสูตร ไม่ได้แก้

ตรวจจริง (region NE2, JSON แคช): `54-008953` ย้ายมิเตอร์ 24 ตัว (เดิม 14), `59-018889` ย้าย 33 ตัว (เดิม 7) Design Check ก่อน→หลัง 5→0 และ 6→0 กลุ่ม, Unbalance สุดท้าย 7.4% และ 14.5% ผ่านเกณฑ์ทั้งคู่, `meter_groups.geojson` ครบเท่ามิเตอร์ใน JSON ดิบ (235 และ 196 ตัว), `steps_applied`/ป้ายสถานะ/สัญญาผลลัพธ์ยังครบ `tools/check_integration.py` PASS ย้าย `PhaseOptimizer_17092026.py` ไป `notUse/` และอัปเดต import ใน `feature_PhaseOptimizer/run_web.py`
